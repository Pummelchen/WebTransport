...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 10 (8 function_body_length, 2 cyclomatic_complexity), all in test files. Every source target is clean -- both codec modules, both handshake paths, both CLI mains and the whole smoke pair -- and every type_body_length finding is fixed. `swift test`: 401 passed; `swift format lint --strict` clean; `./Swift/run-library-smoke.sh` passes. The CI gate is NOT yet wired. Remaining: eight test fixtures.
- commit: 
