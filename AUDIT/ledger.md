...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 23 (17 function_body_length, 6 cyclomatic_complexity; no complexity errors left). Every type_body_length finding is fixed; both codec modules, both handshake paths and both CLI mains are clean. `QUICFrame`'s two 19-way switches are split without losing the outer switch's compile-time exhaustiveness, and the round-trip test covers all nineteen frame types. `swift test`: 401 passed. `swift format lint --strict`: clean. The CI gate is NOT yet wired. Remaining: the LibrarySmoke executables and test fixtures.
- commit: 
