...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 16 (12 function_body_length, 4 cyclomatic_complexity). Every type_body_length finding is fixed; both codec modules, both handshake paths, both CLI mains and the smoke server are clean. `swift test`: 401 passed, `swift format lint --strict` clean, `./Swift/run-library-smoke.sh` passes. The CI gate is NOT yet wired. Remaining: the LibrarySmoke client runner/scenarios and eight test fixtures.
- commit: 
