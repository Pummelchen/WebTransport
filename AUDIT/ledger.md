...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 20. Every type_body_length finding is fixed; both codec modules, both handshake paths, both CLI mains, the smoke client's parser and the smoke server's envelope handler are clean. `QUICFrame`'s two 19-way switches are split without losing the outer switch's compile-time exhaustiveness, and the round-trip test covers all nineteen frame types. Verified: `swift test` 401 passed, `swift format lint --strict` clean, and `Swift/run-library-smoke.sh` passes (the direct check for the smoke server change). The CI gate is NOT yet wired. Remaining: the LibrarySmoke scenarios/runner and test fixtures.
- commit: 
