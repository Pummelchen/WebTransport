...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 13 (10 function_body_length, 3 cyclomatic_complexity). Every type_body_length finding is fixed; both codec modules, both handshake paths, both CLI mains, the smoke server, the smoke client's runner and its interleaved-stream scenario are clean. `swift test`: 401 passed; `swift format lint --strict` clean; `./Swift/run-library-smoke.sh` passes. The CI gate is NOT yet wired. Remaining: the flow-control smoke scenario and nine test fixtures.
- commit: 
