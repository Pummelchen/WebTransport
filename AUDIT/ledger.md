...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 5, all function_body_length in test files. Every type_body_length finding is fixed, every cyclomatic_complexity finding is fixed, and every source target is clean. `swift test`: 401 passed; `swift format lint --strict` clean; `./Swift/run-library-smoke.sh` passes. The CI gate is NOT yet wired -- it can be once the count reaches zero. Remaining: four test fixtures, one of them 214 lines.
- commit: 
