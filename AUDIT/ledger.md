...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 39, all function-level (30 function_body_length, 9 cyclomatic_complexity). Every type_body_length finding is fixed; all of WebTransportHTTP3Core and WebTransportQUICCore is clean. `QUICFrame`'s two 19-way switches are split into grouped dispatch without losing the compile-time exhaustiveness of the outer switch, and the round-trip test now covers every frame type instead of 11 of 19 (401 tests). `swift test`: 401 passed. `swift format lint --strict`: clean. The CI gate is NOT yet wired: it stays unwired until it can be green.
- commit: 
