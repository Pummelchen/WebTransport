...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 30 (21 function_body_length, 9 cyclomatic_complexity). Every type_body_length finding is fixed; WebTransportHTTP3Core, WebTransportQUICCore, WebTransportNetworkSession, WebTransportQUICServer, WebTransportQUICClient and the conformance CLI are clean. `QUICFrame`'s two 19-way switches are split without losing the outer switch's compile-time exhaustiveness, and the round-trip test covers all nineteen frame types. `swift test`: 401 passed. `swift format lint --strict`: clean. The CI gate is NOT yet wired. Remaining: the CLI/smoke executables and test fixtures.
- commit: 
