...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 43, all function-level (32 function_body_length, 11 cyclomatic_complexity). Every type_body_length finding is fixed. All six WebTransportHTTP3Core findings and three of the four WebTransportQUICCore ones are fixed (flow-control capsule parser, the five QPACK field-line representations, the CONNECT-stream capsule loop, the server session-request validator, the control-stream settings/GOAWAY scan, the packet-range insertion, the STREAM-frame receive path, the long-header decoder). `swift test`: 400 passed. `swift format lint --strict`: clean. The CI gate is NOT yet wired: it stays unwired until it can be green. `QUICFrame.encode`/`decode` are the deliberate remainder: a 19-way exhaustive switch whose case count alone exceeds the threshold, and splitting it naively would cost the compile-time exhaustiveness a wire codec needs. The plan for that one is to keep the switch exhaustive over grouped case lists and extend the round-trip test to all nineteen frame types first, so the guard the grouping removes is replaced by a test rather than by nothing; it is tracked here rather than as a separate task because it is a shape decision inside this task's scope.
- commit: 
