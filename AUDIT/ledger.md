...
- evidence after: Unconfigured `swiftlint lint`: 556 findings -> 71 with the committed config. Remaining: 39 function_body_length, 18 cyclomatic_complexity, 11 type_body_length (68 structural, S2 refactors to do), 3 function_parameter_count (test helpers). Fixed since the config landed: 10 line_length, 2 nesting, 2 large_tuple, plus the force-unwrap/force-try/naming/doc-comment set. `swift test` 400 passed; `swift format lint --strict` clean. The CI gate is still not wired, because it cannot be green until the 68 structural findings are refactored.
- commit: 
