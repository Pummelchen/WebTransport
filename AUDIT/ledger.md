...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 47, all function-level (34 function_body_length, 13 cyclomatic_complexity). Every type_body_length finding is fixed. All six WebTransportHTTP3Core findings are fixed: the flow-control capsule parser, the five QPACK field-line representations, the CONNECT-stream capsule loop, the server session-request validator, and the control-stream settings/GOAWAY scan. One file had to be split to stay under the file-length limit (`QPACKStaticTable.swift`). `swift test`: 400 passed. `swift format lint --strict`: clean. The CI gate is NOT yet wired: it stays unwired until it can be green.
- commit: 
