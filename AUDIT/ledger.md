...
- evidence after: Configured `swiftlint --strict`: 556 findings -> 14 (11 function_body_length, 3 cyclomatic_complexity). Every type_body_length finding is fixed; both codec modules, both handshake paths, both CLI mains, the smoke server and the smoke client's runner are clean. `swift test`: 401 passed; `swift format lint --strict` clean; `./Swift/run-library-smoke.sh` passes. The CI gate is NOT yet wired. Remaining: the LibrarySmoke client scenarios and nine test fixtures.
- commit: 
