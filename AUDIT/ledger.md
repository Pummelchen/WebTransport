...
- evidence after: `100% tests passed out of 97`, `check-format.sh`: all 312 C sources match. Deliberate violation: removing the refusal fails with `FAIL a capacity this build cannot fill is unsupported: want unsupported, got ok` and `FAIL and the endpoint keeps the capacity it had: want 0, got 4096` (2 of 181 checks, exit 8); restoring leaves the suite green. Not fixed by implementing the dynamic table -- that is a feature, not an audit repair, and the 247 tested lines of encoder-stream decoder are left in the tree with the wiring recorded as the remaining work rather than deleted. SUPERSEDED 18 September 2026 by WT-265, which built the feature this finding pointed at rather than leaving the API refusing it: the peer's encoder stream is now applied (`wt_http3_endpoint_on_qpack_encoder_bytes`) and its insertions are acknowledged (`wt_http3_endpoint_write_qpack_decoder_acks`), so a non-zero capacity means what it says. What survives from this finding is the refusal to CHANGE a capacity after the peer has inserted against it (WT_ERR_STATE), which is the part that protects those insertions from being silently discarded.
- commit: 74161c1
