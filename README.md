...
- `WebTransportHTTP3Core` includes deterministic WebTransport ALPN/settings/session-policy rejection paths and a `LibrarySmokeClient`/`LibrarySmokeServer` matrix for close/drain, rejection, backpressure, ordering, and multi-session scenarios.
- `NativeQUICCoreSpike` exercises UDP frame exchange, QUIC packet protection, TLS/QUIC handshake state, HTTP/3 control/request handling, WebTransport session establishment, and the library smoke matrix without interactive security prompts.
