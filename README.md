...
| --- | --- | ---: |
| Swift | Active implementation. Protocol core, package product, deterministic client/server CLI facade, packet-protected QUIC Initial CRYPTO flight plus protected HTTP/3 WebTransport CONNECT/DATAGRAM session probe over UDP, explicit TLS/QUIC application-key readiness gating, conformance scenarios, and release packaging are present. | 94% |
...
Important limitation: the Swift client/server CLI now validates a packet-protected QUIC Initial CRYPTO flight and a protected HTTP/3 WebTransport CONNECT/DATAGRAM session probe over UDP, but it is still a deterministic runtime probe rather than a complete external QUIC/TLS/HTTP/3 network stack.
