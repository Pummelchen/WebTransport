...
echo "           fixed seven real defects: the peer's max_idle_timeout read as microseconds instead of"
echo "           milliseconds, CRYPTO frames refused in the 1-RTT space, a post-handshake NewSessionTicket"
echo "           failing the handshake, a report that called a killed connection ok, a client that omitted"
echo "           the mandatory SETTINGS_H3_DATAGRAM and every pre-rename version codepoint, a client that"
echo "           omitted initial_max_stream_data_bidi_remote (so a conformant peer may answer nothing at"
echo "           all), and a server that sent no control stream and no SETTINGS frame whatsoever."
echo "           quiche does not"
