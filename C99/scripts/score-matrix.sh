...
echo "           (WT-153): a third-party client completes the handshake with wt-server-c99 and its"
echo "           CONNECT is then mis-decoded, which found the authority comparison and a QPACK value"
echo "           bug (WT-154) in one run. The VPS matrix's five implementations still need a host"
