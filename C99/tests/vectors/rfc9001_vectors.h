/* Generated by tests/vectors/extract_rfc9001_keys.py -- do not edit.
 *
 * The key material of RFC 9001 appendix A, read from the RFC's own hex and
 * checked against arithmetic that does not depend on the labels before being
 * written: the Initial secret is re-derived from the version-1 salt and the
 * destination connection ID, and every key, IV and header protection key is
 * re-derived from its traffic secret with the QUIC labels. A block read from
 * the wrong place fails the size check or the re-derivation and generation
 * stops rather than writing a vector a wrong implementation would pass.
 */

#ifndef WT_RFC9001_VECTORS_H
#define WT_RFC9001_VECTORS_H

#include <stdint.h>

/* RFC 9001 A.1: HKDF-Extract(version-1 salt, destination connection ID). */
#define WT_RFC9001_INITIAL_SECRET_LEN 32
static const uint8_t WT_RFC9001_INITIAL_SECRET[WT_RFC9001_INITIAL_SECRET_LEN] = {
    0x7d, 0xb5, 0xdf, 0x06, 0xe7, 0xa6, 0x9e, 0x43, 0x24, 0x96, 0xad, 0xed,
    0xb0, 0x08, 0x51, 0x92, 0x35, 0x95, 0x22, 0x15, 0x96, 0xae, 0x2a, 0xe9,
    0xfb, 0x81, 0x15, 0xc1, 0xe9, 0xed, 0x0a, 0x44,
};

/* RFC 9001 A.1: the client's Initial secret. */
#define WT_RFC9001_CLIENT_INITIAL_SECRET_LEN 32
static const uint8_t WT_RFC9001_CLIENT_INITIAL_SECRET[WT_RFC9001_CLIENT_INITIAL_SECRET_LEN] = {
    0xc0, 0x0c, 0xf1, 0x51, 0xca, 0x5b, 0xe0, 0x75, 0xed, 0x0e, 0xbf, 0xb5,
    0xc8, 0x03, 0x23, 0xc4, 0x2d, 0x6b, 0x7d, 0xb6, 0x78, 0x81, 0x28, 0x9a,
    0xf4, 0x00, 0x8f, 0x1f, 0x6c, 0x35, 0x7a, 0xea,
};

/* RFC 9001 A.1: the server's Initial secret. */
#define WT_RFC9001_SERVER_INITIAL_SECRET_LEN 32
static const uint8_t WT_RFC9001_SERVER_INITIAL_SECRET[WT_RFC9001_SERVER_INITIAL_SECRET_LEN] = {
    0x3c, 0x19, 0x98, 0x28, 0xfd, 0x13, 0x9e, 0xfd, 0x21, 0x6c, 0x15, 0x5a,
    0xd8, 0x44, 0xcc, 0x81, 0xfb, 0x82, 0xfa, 0x8d, 0x74, 0x46, 0xfa, 0x7d,
    0x78, 0xbe, 0x80, 0x3a, 0xcd, 0xda, 0x95, 0x1b,
};

/* RFC 9001 A.1: quic key for the client's Initial keys. */
#define WT_RFC9001_CLIENT_INITIAL_KEY_LEN 16
static const uint8_t WT_RFC9001_CLIENT_INITIAL_KEY[WT_RFC9001_CLIENT_INITIAL_KEY_LEN] = {
    0x1f, 0x36, 0x96, 0x13, 0xdd, 0x76, 0xd5, 0x46, 0x77, 0x30, 0xef, 0xcb,
    0xe3, 0xb1, 0xa2, 0x2d,
};

/* RFC 9001 A.1: quic iv for the client's Initial keys. */
#define WT_RFC9001_CLIENT_INITIAL_IV_LEN 12
static const uint8_t WT_RFC9001_CLIENT_INITIAL_IV[WT_RFC9001_CLIENT_INITIAL_IV_LEN] = {
    0xfa, 0x04, 0x4b, 0x2f, 0x42, 0xa3, 0xfd, 0x3b, 0x46, 0xfb, 0x25, 0x5c,
};

/* RFC 9001 A.1: quic hp for the client's Initial keys. */
#define WT_RFC9001_CLIENT_INITIAL_HP_LEN 16
static const uint8_t WT_RFC9001_CLIENT_INITIAL_HP[WT_RFC9001_CLIENT_INITIAL_HP_LEN] = {
    0x9f, 0x50, 0x44, 0x9e, 0x04, 0xa0, 0xe8, 0x10, 0x28, 0x3a, 0x1e, 0x99,
    0x33, 0xad, 0xed, 0xd2,
};

/* RFC 9001 A.1: quic key for the server's Initial keys. */
#define WT_RFC9001_SERVER_INITIAL_KEY_LEN 16
static const uint8_t WT_RFC9001_SERVER_INITIAL_KEY[WT_RFC9001_SERVER_INITIAL_KEY_LEN] = {
    0xcf, 0x3a, 0x53, 0x31, 0x65, 0x3c, 0x36, 0x4c, 0x88, 0xf0, 0xf3, 0x79,
    0xb6, 0x06, 0x7e, 0x37,
};

/* RFC 9001 A.1: quic iv for the server's Initial keys. */
#define WT_RFC9001_SERVER_INITIAL_IV_LEN 12
static const uint8_t WT_RFC9001_SERVER_INITIAL_IV[WT_RFC9001_SERVER_INITIAL_IV_LEN] = {
    0x0a, 0xc1, 0x49, 0x3c, 0xa1, 0x90, 0x58, 0x53, 0xb0, 0xbb, 0xa0, 0x3e,
};

/* RFC 9001 A.1: quic hp for the server's Initial keys. */
#define WT_RFC9001_SERVER_INITIAL_HP_LEN 16
static const uint8_t WT_RFC9001_SERVER_INITIAL_HP[WT_RFC9001_SERVER_INITIAL_HP_LEN] = {
    0xc2, 0x06, 0xb8, 0xd9, 0xb9, 0xf0, 0xf3, 0x76, 0x44, 0x43, 0x0b, 0x49,
    0x0e, 0xea, 0xa3, 0x14,
};

/* RFC 9001 A.5: a traffic secret for ChaCha20-Poly1305. */
#define WT_RFC9001_CHACHA_SECRET_LEN 32
static const uint8_t WT_RFC9001_CHACHA_SECRET[WT_RFC9001_CHACHA_SECRET_LEN] = {
    0x9a, 0xc3, 0x12, 0xa7, 0xf8, 0x77, 0x46, 0x8e, 0xbe, 0x69, 0x42, 0x27,
    0x48, 0xad, 0x00, 0xa1, 0x54, 0x43, 0xf1, 0x82, 0x03, 0xa0, 0x7d, 0x60,
    0x60, 0xf6, 0x88, 0xf3, 0x0f, 0x21, 0x63, 0x2b,
};

/* RFC 9001 A.5: quic key, 32 bytes because the AEAD's key is. */
#define WT_RFC9001_CHACHA_KEY_LEN 32
static const uint8_t WT_RFC9001_CHACHA_KEY[WT_RFC9001_CHACHA_KEY_LEN] = {
    0xc6, 0xd9, 0x8f, 0xf3, 0x44, 0x1c, 0x3f, 0xe1, 0xb2, 0x18, 0x20, 0x94,
    0xf6, 0x9c, 0xaa, 0x2e, 0xd4, 0xb7, 0x16, 0xb6, 0x54, 0x88, 0x96, 0x0a,
    0x7a, 0x98, 0x49, 0x79, 0xfb, 0x23, 0xe1, 0xc8,
};

/* RFC 9001 A.5: quic iv. */
#define WT_RFC9001_CHACHA_IV_LEN 12
static const uint8_t WT_RFC9001_CHACHA_IV[WT_RFC9001_CHACHA_IV_LEN] = {
    0xe0, 0x45, 0x9b, 0x34, 0x74, 0xbd, 0xd0, 0xe4, 0x4a, 0x41, 0xc1, 0x44,
};

/* RFC 9001 A.5: quic hp, 32 bytes for ChaCha20. */
#define WT_RFC9001_CHACHA_HP_LEN 32
static const uint8_t WT_RFC9001_CHACHA_HP[WT_RFC9001_CHACHA_HP_LEN] = {
    0x25, 0xa2, 0x82, 0xb9, 0xe8, 0x2f, 0x06, 0xf2, 0x1f, 0x48, 0x89, 0x17,
    0xa4, 0xfc, 0x8f, 0x1b, 0x73, 0x57, 0x36, 0x85, 0x60, 0x85, 0x97, 0xd0,
    0xef, 0xcb, 0x07, 0x6b, 0x0a, 0xb7, 0xa7, 0xa4,
};

/* RFC 9001 A.5: quic ku, the secret the next key update uses. */
#define WT_RFC9001_CHACHA_KEY_UPDATE_LEN 32
static const uint8_t WT_RFC9001_CHACHA_KEY_UPDATE[WT_RFC9001_CHACHA_KEY_UPDATE_LEN] = {
    0x12, 0x23, 0x50, 0x47, 0x55, 0x03, 0x6d, 0x55, 0x63, 0x42, 0xee, 0x93,
    0x61, 0xd2, 0x53, 0x42, 0x1a, 0x82, 0x6c, 0x9e, 0xcd, 0xf3, 0xc7, 0x14,
    0x86, 0x84, 0xb3, 0x6b, 0x71, 0x48, 0x81, 0xf9,
};

/* RFC 9001 A.2: the client Initial header before header protection. */
#define WT_RFC9001_CLIENT_INITIAL_HEADER_PLAIN_LEN 22
static const uint8_t WT_RFC9001_CLIENT_INITIAL_HEADER_PLAIN[WT_RFC9001_CLIENT_INITIAL_HEADER_PLAIN_LEN] = {
    0xc3, 0x00, 0x00, 0x00, 0x01, 0x08, 0x83, 0x94, 0xc8, 0xf0, 0x3e, 0x51,
    0x57, 0x08, 0x00, 0x00, 0x44, 0x9e, 0x00, 0x00, 0x00, 0x02,
};

/* RFC 9001 A.2: the same header after header protection. */
#define WT_RFC9001_CLIENT_INITIAL_HEADER_PROTECTED_LEN 22
static const uint8_t WT_RFC9001_CLIENT_INITIAL_HEADER_PROTECTED[WT_RFC9001_CLIENT_INITIAL_HEADER_PROTECTED_LEN] = {
    0xc0, 0x00, 0x00, 0x00, 0x01, 0x08, 0x83, 0x94, 0xc8, 0xf0, 0x3e, 0x51,
    0x57, 0x08, 0x00, 0x00, 0x44, 0x9e, 0x7b, 0x9a, 0xec, 0x34,
};

/* RFC 9001 A.2: the header protection sample, at pn_offset + 4. */
#define WT_RFC9001_CLIENT_INITIAL_SAMPLE_LEN 16
static const uint8_t WT_RFC9001_CLIENT_INITIAL_SAMPLE[WT_RFC9001_CLIENT_INITIAL_SAMPLE_LEN] = {
    0xd1, 0xb1, 0xc9, 0x8d, 0xd7, 0x68, 0x9f, 0xb8, 0xec, 0x11, 0xd2, 0x42,
    0xb1, 0x23, 0xdc, 0x9b,
};

/* RFC 9001 A.2: the five-byte header protection mask. */
#define WT_RFC9001_CLIENT_INITIAL_MASK_LEN 5
static const uint8_t WT_RFC9001_CLIENT_INITIAL_MASK[WT_RFC9001_CLIENT_INITIAL_MASK_LEN] = {
    0x43, 0x7b, 0x9a, 0xec, 0x36,
};

/* RFC 9001 A.2: the complete protected client Initial packet, 1200 bytes. */
#define WT_RFC9001_CLIENT_INITIAL_PACKET_LEN 1200
static const uint8_t WT_RFC9001_CLIENT_INITIAL_PACKET[WT_RFC9001_CLIENT_INITIAL_PACKET_LEN] = {
    ,
};

/* RFC 9001 A.3: the server Initial header before header protection. */
#define WT_RFC9001_SERVER_INITIAL_HEADER_PLAIN_LEN 20
static const uint8_t WT_RFC9001_SERVER_INITIAL_HEADER_PLAIN[WT_RFC9001_SERVER_INITIAL_HEADER_PLAIN_LEN] = {
    0xc1, 0x00, 0x00, 0x00, 0x01, 0x00, 0x08, 0xf0, 0x67, 0xa5, 0x50, 0x2a,
    0x42, 0x62, 0xb5, 0x00, 0x40, 0x75, 0x00, 0x01,
};

/* RFC 9001 A.3: the same header after header protection. */
#define WT_RFC9001_SERVER_INITIAL_HEADER_PROTECTED_LEN 20
static const uint8_t WT_RFC9001_SERVER_INITIAL_HEADER_PROTECTED[WT_RFC9001_SERVER_INITIAL_HEADER_PROTECTED_LEN] = {
    0xcf, 0x00, 0x00, 0x00, 0x01, 0x00, 0x08, 0xf0, 0x67, 0xa5, 0x50, 0x2a,
    0x42, 0x62, 0xb5, 0x00, 0x40, 0x75, 0xc0, 0xd9,
};

/* RFC 9001 A.3: the header protection sample, at pn_offset + 4. */
#define WT_RFC9001_SERVER_INITIAL_SAMPLE_LEN 16
static const uint8_t WT_RFC9001_SERVER_INITIAL_SAMPLE[WT_RFC9001_SERVER_INITIAL_SAMPLE_LEN] = {
    0x2c, 0xd0, 0x99, 0x1c, 0xd2, 0x5b, 0x0a, 0xac, 0x40, 0x6a, 0x58, 0x16,
    0xb6, 0x39, 0x41, 0x00,
};

/* RFC 9001 A.3: the five-byte header protection mask. */
#define WT_RFC9001_SERVER_INITIAL_MASK_LEN 5
static const uint8_t WT_RFC9001_SERVER_INITIAL_MASK[WT_RFC9001_SERVER_INITIAL_MASK_LEN] = {
    0x2e, 0xc0, 0xd8, 0x35, 0x6a,
};

/* RFC 9001 A.3: the complete protected server Initial packet, 135 bytes. */
#define WT_RFC9001_SERVER_INITIAL_PACKET_LEN 135
static const uint8_t WT_RFC9001_SERVER_INITIAL_PACKET[WT_RFC9001_SERVER_INITIAL_PACKET_LEN] = {
    ,
};

/* RFC 9001 A.5: the packet number of the ChaCha20 short header packet. */
#define WT_RFC9001_CHACHA_PN UINT64_C(654360564)

/* RFC 9001 A.5: the AEAD nonce, the IV with the packet number XORed in. */
#define WT_RFC9001_CHACHA_NONCE_LEN 12
static const uint8_t WT_RFC9001_CHACHA_NONCE[WT_RFC9001_CHACHA_NONCE_LEN] = {
    0xe0, 0x45, 0x9b, 0x34, 0x74, 0xbd, 0xd0, 0xe4, 0x6d, 0x41, 0x7e, 0xb0,
};

/* RFC 9001 A.5: the short header before header protection, empty DCID. */
#define WT_RFC9001_CHACHA_HEADER_PLAIN_LEN 4
static const uint8_t WT_RFC9001_CHACHA_HEADER_PLAIN[WT_RFC9001_CHACHA_HEADER_PLAIN_LEN] = {
    0x42, 0x00, 0xbf, 0xf4,
};

/* RFC 9001 A.5: the short header after header protection. */
#define WT_RFC9001_CHACHA_HEADER_PROTECTED_LEN 4
static const uint8_t WT_RFC9001_CHACHA_HEADER_PROTECTED[WT_RFC9001_CHACHA_HEADER_PROTECTED_LEN] = {
    0x4c, 0xfe, 0x41, 0x89,
};

/* RFC 9001 A.5: the header protection sample, at pn_offset + 4. */
#define WT_RFC9001_CHACHA_SAMPLE_LEN 16
static const uint8_t WT_RFC9001_CHACHA_SAMPLE[WT_RFC9001_CHACHA_SAMPLE_LEN] = {
    0x5e, 0x5c, 0xd5, 0x5c, 0x41, 0xf6, 0x90, 0x80, 0x57, 0x5d, 0x79, 0x99,
    0xc2, 0x5a, 0x5b, 0xfb,
};

/* RFC 9001 A.5: the five-byte header protection mask. */
#define WT_RFC9001_CHACHA_MASK_LEN 5
static const uint8_t WT_RFC9001_CHACHA_MASK[WT_RFC9001_CHACHA_MASK_LEN] = {
    0xae, 0xfe, 0xfe, 0x7d, 0x03,
};

/* RFC 9001 A.5: the payload before encryption: one PING frame. */
#define WT_RFC9001_CHACHA_PAYLOAD_PLAIN_LEN 1
static const uint8_t WT_RFC9001_CHACHA_PAYLOAD_PLAIN[WT_RFC9001_CHACHA_PAYLOAD_PLAIN_LEN] = {
    0x01,
};

/* RFC 9001 A.5: the payload after encryption, tag included. */
#define WT_RFC9001_CHACHA_CIPHERTEXT_LEN 17
static const uint8_t WT_RFC9001_CHACHA_CIPHERTEXT[WT_RFC9001_CHACHA_CIPHERTEXT_LEN] = {
    0x65, 0x5e, 0x5c, 0xd5, 0x5c, 0x41, 0xf6, 0x90, 0x80, 0x57, 0x5d, 0x79,
    0x99, 0xc2, 0x5a, 0x5b, 0xfb,
};

/* RFC 9001 A.5: the protected ChaCha20-Poly1305 short header packet. */
#define WT_RFC9001_CHACHA_PACKET_LEN 21
static const uint8_t WT_RFC9001_CHACHA_PACKET[WT_RFC9001_CHACHA_PACKET_LEN] = {
    0x4c, 0xfe, 0x41, 0x89, 0x65, 0x5e, 0x5c, 0xd5, 0x5c, 0x41, 0xf6, 0x90,
    0x80, 0x57, 0x5d, 0x79, 0x99, 0xc2, 0x5a, 0x5b, 0xfb,
};

#endif /* WT_RFC9001_VECTORS_H */
