...
static void test_primitives(void) {
  ...
  WT_EXPECT_BYTES("is the RFC's", WT_RFC7748_OUTPUT_1, out, WT_TLS_X25519_KEY_LEN);

  WT_EXPECT_OK("the second scalar-multiplication vector",
               wt_tls_x25519(WT_RFC7748_SCALAR_2, WT_RFC7748_U_COORDINATE_2, out));
  WT_EXPECT_BYTES("is the RFC's", WT_RFC7748_OUTPUT_2, out, WT_TLS_X25519_KEY_LEN);

  /* The two are not the same computation, which is what makes them two tests. */
  WT_EXPECT_INT("the two vectors differ", 0,
                memcmp(WT_RFC7748_OUTPUT_1, WT_RFC7748_OUTPUT_2, WT_TLS_X25519_KEY_LEN) == 0 ? 1
                                                                                             : 0);

  /* A public key is X25519(private, 9), and the RFC prints both of Alice's and Bob's. */
  WT_EXPECT_OK("Alice's public key",
               wt_tls_key_share_public_key(WT_TLS_GROUP_X25519, WT_RFC7748_ALICE_PRIVATE, out));
  WT_EXPECT_BYTES("is the RFC's", WT_RFC7748_ALICE_PUBLIC, out, WT_TLS_X25519_KEY_LEN);
  WT_EXPECT_OK("Bob's public key",
               wt_tls_key_share_public_key(WT_TLS_GROUP_X25519, WT_RFC7748_BOB_PRIVATE, out));
  WT_EXPECT_BYTES("is the RFC's", WT_RFC7748_BOB_PUBLIC, out, WT_TLS_X25519_KEY_LEN);
  ...
  WT_EXPECT_STATUS(
      "an unsupported group is refused", WT_ERR_UNSUPPORTED,
      wt_tls_key_share_public_key(WT_TLS_GROUP_SECP256R1, WT_RFC7748_ALICE_PRIVATE, out));
  ...
  WT_EXPECT_STATUS(
      "a NULL output is refused", WT_ERR_INVALID_ARGUMENT,
      wt_tls_key_share_public_key(WT_TLS_GROUP_X25519, WT_RFC7748_ALICE_PRIVATE, NULL));
...
static void test_key_exchange(void) {
  ...
  WT_EXPECT_OK("Alice's side", wt_tls_key_share_shared_secret(
                                   WT_TLS_GROUP_X25519, WT_RFC7748_ALICE_PRIVATE,
                                   WT_RFC7748_BOB_PUBLIC, WT_TLS_X25519_KEY_LEN, alice_secret));
  WT_EXPECT_BYTES("is the RFC's shared secret", WT_RFC7748_SHARED_SECRET, alice_secret,
                  WT_TLS_X25519_KEY_LEN);
  WT_EXPECT_OK("Bob's side", wt_tls_key_share_shared_secret(
                                 WT_TLS_GROUP_X25519, WT_RFC7748_BOB_PRIVATE,
                                 WT_RFC7748_ALICE_PUBLIC, WT_TLS_X25519_KEY_LEN, bob_secret));
  WT_EXPECT_BYTES("and agrees", alice_secret, bob_secret, WT_TLS_X25519_KEY_LEN);
   ...
   */
  {
    ...
    WT_EXPECT_OK("the RFC 8448 client's public key",
                 wt_tls_key_share_public_key(WT_TLS_GROUP_X25519, WT_RFC8448_CLIENT_X25519_PRIVATE,
                                             public_key));
    WT_EXPECT_BYTES("is the one RFC 8448 sent", WT_RFC8448_CLIENT_X25519_PUBLIC, public_key,
                    WT_TLS_X25519_KEY_LEN);
    WT_EXPECT_OK("the RFC 8448 server's public key",
                 wt_tls_key_share_public_key(WT_TLS_GROUP_X25519, WT_RFC8448_SERVER_X25519_PRIVATE,
                                             public_key));
    WT_EXPECT_BYTES("is the one RFC 8448 sent", WT_RFC8448_SERVER_X25519_PUBLIC, public_key,
                    WT_TLS_X25519_KEY_LEN);
    ...
    WT_EXPECT_OK("the handshake's shared secret",
                 wt_tls_key_share_shared_secret(
                     ...
                     WT_RFC8448_SERVER_X25519_PUBLIC, WT_TLS_X25519_KEY_LEN, ecdhe));
    WT_EXPECT_BYTES("is the ECDHE value its key schedule consumed", WT_RFC8448_ECDHE, ecdhe,
                    WT_TLS_X25519_KEY_LEN);
  }
   ...
   * checks both the status and that the buffer was cleared. */
  {
    ...
    WT_EXPECT_STATUS("a small-order public key is refused", WT_ERR_PROTOCOL,
                     wt_tls_key_share_shared_secret(WT_TLS_GROUP_X25519, WT_RFC7748_ALICE_PRIVATE,
                                                    WT_SMALL_ORDER_KEY, sizeof(WT_SMALL_ORDER_KEY),
                                                    secret));
    ...
    WT_EXPECT_STATUS("the primitive refuses it too", WT_ERR_PROTOCOL,
                     wt_tls_x25519(WT_RFC7748_ALICE_PRIVATE, WT_SMALL_ORDER_KEY, secret));
  }
  ...
  WT_EXPECT_STATUS("a NULL peer key is refused", WT_ERR_INVALID_ARGUMENT,
                   wt_tls_key_share_shared_secret(WT_TLS_GROUP_X25519, WT_RFC7748_ALICE_PRIVATE,
                                                  NULL, WT_TLS_X25519_KEY_LEN, alice_secret));
  WT_EXPECT_STATUS("an unsupported group is refused", WT_ERR_UNSUPPORTED,
                   wt_tls_key_share_shared_secret(WT_TLS_GROUP_SECP256R1, WT_RFC7748_ALICE_PRIVATE,
...
static void test_generation(void) {
  ...
  WT_EXPECT_OK("a key pair generates",
               wt_tls_key_share_generate(WT_TLS_GROUP_X25519, first_private, first_public));
  WT_EXPECT_OK("and another",
               wt_tls_key_share_generate(WT_TLS_GROUP_X25519, second_private, second_public));
  /* Two generations agreeing would mean the generator is not generating. */
  WT_EXPECT_INT("the two private keys differ", 0,
                memcmp(first_private, second_private, WT_TLS_X25519_KEY_LEN) == 0 ? 1 : 0);
  WT_EXPECT_INT("and so do the public keys", 0,
                memcmp(first_public, second_public, WT_TLS_X25519_KEY_LEN) == 0 ? 1 : 0);
  ...
  WT_EXPECT_OK("the public key is reproducible from the private key",
               wt_tls_key_share_public_key(WT_TLS_GROUP_X25519, first_private, from_private));
  WT_EXPECT_BYTES("byte for byte", first_public, from_private, WT_TLS_X25519_KEY_LEN);
  WT_EXPECT_OK("the first side's secret",
               wt_tls_key_share_shared_secret(WT_TLS_GROUP_X25519, first_private, second_public,
                                              WT_TLS_X25519_KEY_LEN, shared_one));
  WT_EXPECT_OK("the second side's secret",
               wt_tls_key_share_shared_secret(WT_TLS_GROUP_X25519, second_private, first_public,
                                              WT_TLS_X25519_KEY_LEN, shared_two));
  WT_EXPECT_BYTES("agreeing", shared_one, shared_two, WT_TLS_X25519_KEY_LEN);
  WT_EXPECT_INT("and not all zero", 1, shared_one[0] == 0U && shared_one[1] == 0U ? 0 : 1);

  WT_EXPECT_STATUS("an unsupported group cannot generate", WT_ERR_UNSUPPORTED,
                   wt_tls_key_share_generate(WT_TLS_GROUP_SECP256R1, first_private, first_public));
  WT_EXPECT_STATUS("a NULL private key is refused", WT_ERR_INVALID_ARGUMENT,
                   wt_tls_key_share_generate(WT_TLS_GROUP_X25519, NULL, first_public));
  WT_EXPECT_STATUS("a NULL public key is refused", WT_ERR_INVALID_ARGUMENT,
                   wt_tls_key_share_generate(WT_TLS_GROUP_X25519, first_private, NULL));
}

static void test_group_registry(void) {
  WT_EXPECT_INT("x25519 is supported", 1, wt_tls_key_share_supported(WT_TLS_GROUP_X25519));
  WT_EXPECT_INT("secp256r1 is not", 0, wt_tls_key_share_supported(WT_TLS_GROUP_SECP256R1));
  WT_EXPECT_INT("nor is an unknown group", 0, wt_tls_key_share_supported(0x1234U));
