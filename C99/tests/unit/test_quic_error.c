...
int main(void) {
  ...
  WT_EXPECT_U64("FRAME_ENCODING_ERROR is 0x07", 0x07U,
                WT_QUIC_FRAME_ENCODING_ERROR);
  WT_EXPECT_U64("TRANSPORT_PARAMETER_ERROR is 0x08", 0x08U,
                WT_QUIC_TRANSPORT_PARAMETER_ERROR);
  WT_EXPECT_U64("CONNECTION_ID_LIMIT_ERROR is 0x09", 0x09U,
                WT_QUIC_CONNECTION_ID_LIMIT_ERROR);
  ...
  WT_EXPECT_U64("CRYPTO_BUFFER_EXCEEDED is 0x0d", 0x0dU,
                WT_QUIC_CRYPTO_BUFFER_EXCEEDED);
  ...
  WT_EXPECT_U64("the crypto range starts at 0x0100", 0x0100U,
                WT_QUIC_CRYPTO_ERROR_BASE);
  ...
  WT_EXPECT_STR("the crypto range", "crypto",
                wt_quic_error_name(0x0178U));
  WT_EXPECT_STR("an unassigned code", "unknown",
                wt_quic_error_name(0x4000U));
