 ...
 */
static void test_a_retransmission_descriptor_is_released_on_acknowledgement(void) {
  connection_pair_t pair;
  uint8_t payload[32];
 ...
 */
static void test_a_lost_control_frame_is_sent_again(void) {
  connection_pair_t pair;
  wt_quic_frame_t ping = wt_quic_frame_make(WT_QUIC_FRAME_KIND_PING);
  uint64_t now = 109000000U;
  unsigned i;
 ...
 */
static void test_an_acknowledged_control_frame_is_not_sent_again(void) {
  connection_pair_t pair;
  uint64_t now = 110000000U;
  uint64_t received_before;
  size_t i;
 ...
 */
static void test_only_retransmittable_frames_keep_a_slot(void) {
  connection_pair_t pair;
  wt_quic_frame_t ping = wt_quic_frame_make(WT_QUIC_FRAME_KIND_PING);
  wt_quic_frame_t ack = wt_quic_frame_make(WT_QUIC_FRAME_KIND_ACK);
  wt_quic_transport_parameters_t params;
...
static void arm_path_pair(connection_pair_t *pair, uint8_t seed) {
  wt_quic_packet_keys_t keys;
  uint8_t secret[WT_SHA256_LEN];
  size_t i;
...
static const recorded_frame_t *witness_frame(const fs_witness_t *witness, wt_quic_frame_type_t kind,
                                             unsigned occurrence) {
  size_t index;
  unsigned seen = 0U;
...
static void test_a_path_is_validated_by_its_own_response(void) {
  connection_pair_t pair;
...
static void test_a_path_that_does_not_answer_is_given_up_on(void) {
  connection_pair_t pair;
