...
int main(void) {
  ...
  WT_EXPECT_U64("1073741823 is four bytes", 4U,
                wt_quic_varint_size(1073741823U));
  WT_EXPECT_U64("1073741824 is eight bytes", 8U,
                wt_quic_varint_size(1073741824U));
  WT_EXPECT_U64("2^62-1 is eight bytes", 8U,
                wt_quic_varint_size(WT_QUIC_VARINT_MAX));
  ...
  WT_EXPECT_U64("2^62 has no encoding", 0U,
                wt_quic_varint_size(UINT64_C(1) << 62));
  WT_EXPECT_U64("UINT64_MAX has no encoding", 0U,
                wt_quic_varint_size(UINT64_MAX));
  ...
  WT_EXPECT_BYTES("494878333 is 0x9d7f3e7d",
                  (const uint8_t *)"\x9d\x7f\x3e\x7d", buffer, 4U);

  written = wt_quic_varint_encode(UINT64_C(151288809941952652), buffer,
                                  sizeof(buffer));
  WT_EXPECT_U64("the RFC's eight-byte example", 8U, written);
  WT_EXPECT_BYTES("is 0xc2197c5eff14e88c",
                  (const uint8_t *)"\xc2\x19\x7c\x5e\xff\x14\xe8\x8c", buffer,
                  8U);
  ...
  WT_EXPECT_BYTES("and is 0xff followed by seven 0xff",
                  (const uint8_t *)"\xff\xff\xff\xff\xff\xff\xff\xff", buffer,
                  8U);

  /* A buffer too small is refused rather than truncated. */
  WT_EXPECT_U64("one byte will not hold two", 0U,
                wt_quic_varint_encode(64U, buffer, 1U));
  WT_EXPECT_U64("a NULL buffer is refused", 0U,
                wt_quic_varint_encode(1U, NULL, 8U));
  WT_EXPECT_U64("an unencodable value is refused", 0U,
                wt_quic_varint_encode(UINT64_C(1) << 62, buffer,
                                      sizeof(buffer)));
  ...
  /* Decoding, including the RFC's four examples read back. */
  {
    static const struct {
      ...
      size_t expected_size;
        ...
        {"0x9d7f3e7d",
         {0x9dU, 0x7fU, 0x3eU, 0x7dU, 0, 0, 0, 0},
         4U,
         494878333U,
         4U},
        {"0xc2197c5eff14e88c",
    ...
    for (i = 0U; i < sizeof(vectors) / sizeof(vectors[0]); i++) {
      ...
      WT_EXPECT_OK(vectors[i].label, wt_quic_varint_decode_sized(&c, &value,
                                                                 &size));
      WT_EXPECT_U64("  value", vectors[i].expected, value);
      WT_EXPECT_U64("  size", (uint64_t)vectors[i].expected_size, (uint64_t)size);
      WT_EXPECT_INT("  and it consumed the whole vector", 1,
                    wt_cursor_at_end(&c));
    }
  }
   ...
   * is a refusal, and the cursor is left failed rather than half-read. */
  {
    ...
    static const uint8_t eight_byte_prefix[7] = {0xc2U, 0x19U, 0x7cU, 0x5eU,
                                                 0xffU, 0x14U, 0xe8U};
    c = wt_cursor_init(two_byte_prefix, sizeof(two_byte_prefix));
    WT_EXPECT_STATUS("a two-byte value with one byte is truncated",
                     WT_ERR_TRUNCATED, wt_quic_varint_decode(&c, &value));
    WT_EXPECT_INT("and the cursor is failed", 1, wt_cursor_failed(&c));
    c = wt_cursor_init(four_byte_prefix, sizeof(four_byte_prefix));
    WT_EXPECT_STATUS("a four-byte value with three bytes is truncated",
                     WT_ERR_TRUNCATED, wt_quic_varint_decode(&c, &value));
    c = wt_cursor_init(eight_byte_prefix, sizeof(eight_byte_prefix));
    WT_EXPECT_STATUS("an eight-byte value with seven bytes is truncated",
                     WT_ERR_TRUNCATED, wt_quic_varint_decode(&c, &value));
  ...
  {
    static const uint8_t padded[8] = {0x80U, 0x00U, 0x00U, 0x01U,
                                      0, 0, 0, 0};
    c = wt_cursor_init(padded, 4U);
    WT_EXPECT_OK("a four-byte encoding of 1 decodes", wt_quic_varint_decode_sized(
                                                          &c, &value, &size));
    WT_EXPECT_U64("as the value 1", 1U, value);
    WT_EXPECT_U64("in four bytes", 4U, size);
    WT_EXPECT_INT("and it is not minimal", 0,
                  wt_quic_varint_is_minimal(value, size));
    WT_EXPECT_INT("while the minimal size of 1 is one byte", 1,
                  wt_quic_varint_is_minimal(1U, 1U));
  }
   ...
   * and at a sample in between. */
  {
    static const uint64_t samples[] = {
        0U,          1U,          63U,          64U,
        255U,        16383U,      16384U,       65535U,
        1073741823U, 1073741824U, UINT64_C(0x1234567890ABCDEF),
        WT_QUIC_VARINT_MAX};
    size_t i;
    for (i = 0U; i < sizeof(samples) / sizeof(samples[0]); i++) {
      ...
      WT_EXPECT_U64("encoding a sample takes its size", (uint64_t)size,
                    (uint64_t)written);
   ...
   * measuring pass and the writing pass agree on the size. */
  {
    ...
    WT_EXPECT_U64("and advanced the writer by four", 4U,
    ...
    WT_EXPECT_BYTES("and produced the vector bytes",
                    (const uint8_t *)"\x9d\x7f\x3e\x7d", buffer, 4U);
  }
