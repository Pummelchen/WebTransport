...
int main(void) {
  ...
  other_address_length = encode_address(WT_UDP_IPV6, 4434U, 0x2aU, other_address, sizeof(other_address));
  ...
  WT_EXPECT_INT("two peers encode differently", 0, memcmp(address, other_address, address_length) == 0);
  ...
  WT_EXPECT_OK("a token builds",
               wt_quic_retry_token_build(k_secret, address, address_length, original, 8U, k_now, token,
  ...
  WT_EXPECT_OK("a twenty-byte original builds",
               wt_quic_retry_token_build(k_secret, address, address_length, original,
                                         WT_QUIC_MAX_CONNECTION_ID_LENGTH, k_now, token, sizeof(token),
                                         &token_length));
  WT_EXPECT_OK("and validates",
               wt_quic_retry_token_validate(k_secret, address, address_length, k_now, 0U, token, token_length,
                                            odcid, sizeof(odcid), &odcid_length));
  WT_EXPECT_U64("with all twenty bytes", (uint64_t)WT_QUIC_MAX_CONNECTION_ID_LENGTH, (uint64_t)odcid_length);
  ...
  WT_EXPECT_STATUS("a token for another address does not validate", WT_ERR_AUTHENTICATION,
                   wt_quic_retry_token_validate(k_secret, other_address, other_address_length, k_now, 0U, token,
                                                token_length, odcid, sizeof(odcid), &odcid_length));
  WT_EXPECT_STATUS("nor for a truncated address", WT_ERR_INVALID_ARGUMENT,
                   wt_quic_retry_token_validate(k_secret, address, 0U, k_now, 0U, token, token_length, odcid,
  ...
  WT_EXPECT_STATUS("a token from another server does not validate", WT_ERR_AUTHENTICATION,
                   wt_quic_retry_token_validate(other_secret, address, address_length, k_now, 0U, token,
                                                token_length, odcid, sizeof(odcid), &odcid_length));
   ...
   * address, the length, the original destination ID and the tag itself. */
  {
    ...
    for (i = 0U; i < sizeof(k_tamper_offsets) / sizeof(k_tamper_offsets[0]); i++) {
      ...
      WT_EXPECT_STATUS("a tampered byte is not this server's token", WT_ERR_AUTHENTICATION,
                       wt_quic_retry_token_validate(k_secret, address, address_length, k_now, 0U, tampered,
                                                    token_length, odcid, sizeof(odcid), &odcid_length));
    }
  }
  ...
  WT_EXPECT_STATUS("a token shorter than the layout is not ours", WT_ERR_AUTHENTICATION,
                   wt_quic_retry_token_validate(k_secret, address, address_length, k_now, 0U, token, 8U, odcid,
                                                sizeof(odcid), &odcid_length));
  WT_EXPECT_STATUS("and neither is one longer than it", WT_ERR_AUTHENTICATION,
                   wt_quic_retry_token_validate(k_secret, address, address_length, k_now, 0U, token,
                                                sizeof(token) + 1U, odcid, sizeof(odcid), &odcid_length));
  ...
  WT_EXPECT_STATUS("a null token is a caller error", WT_ERR_INVALID_ARGUMENT,
                   wt_quic_retry_token_validate(k_secret, address, address_length, k_now, 0U, NULL, 0U, odcid,
                                                sizeof(odcid), &odcid_length));
  ...
  WT_EXPECT_STATUS("a token inside the maximum age validates", WT_OK,
                   wt_quic_retry_token_validate(k_secret, address, address_length, k_now + 1000U, 1000U, token,
                                                token_length, odcid, sizeof(odcid), &odcid_length));
  WT_EXPECT_STATUS("one older than it does not", WT_ERR_STATE,
                   wt_quic_retry_token_validate(k_secret, address, address_length, k_now + 1001U, 1000U, token,
                                                token_length, odcid, sizeof(odcid), &odcid_length));
  WT_EXPECT_STATUS("and a token from the future is not spent on either", WT_ERR_STATE,
                   wt_quic_retry_token_validate(k_secret, address, address_length, k_now - 1U, 1000U, token,
                                                token_length, odcid, sizeof(odcid), &odcid_length));
  WT_EXPECT_STATUS("no bound means no expiry check", WT_OK,
                   wt_quic_retry_token_validate(k_secret, address, address_length, k_now + 1000000000U, 0U,
                                                token, token_length, odcid, sizeof(odcid), &odcid_length));
  ...
  WT_EXPECT_STATUS("a capacity that cannot hold the token is refused", WT_ERR_LIMIT,
                   wt_quic_retry_token_build(k_secret, address, address_length, original, 8U, k_now, token, 16U,
                                             &token_length));
  WT_EXPECT_STATUS("no original destination ID is refused", WT_ERR_INVALID_ARGUMENT,
                   wt_quic_retry_token_build(k_secret, address, address_length, original, 0U, k_now, token,
                                             sizeof(token), &token_length));
  WT_EXPECT_STATUS("and one longer than a connection ID", WT_ERR_INVALID_ARGUMENT,
                   wt_quic_retry_token_build(k_secret, address, address_length, original, 21U, k_now, token,
                                             sizeof(token), &token_length));
  WT_EXPECT_STATUS("an address longer than the form allows", WT_ERR_INVALID_ARGUMENT,
                   wt_quic_retry_token_build(k_secret, address, WT_QUIC_RETRY_TOKEN_ADDRESS_MAX + 1U, original,
                                             8U, k_now, token, sizeof(token), &token_length));
  WT_EXPECT_STATUS("a null secret", WT_ERR_INVALID_ARGUMENT,
                   wt_quic_retry_token_build(NULL, address, address_length, original, 8U, k_now, token,
                                             sizeof(token), &token_length));
