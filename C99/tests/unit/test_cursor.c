...
int main(void) {
  static const uint8_t bytes[8] = {0x01U, 0x02U, 0x03U, 0x04U,
  ...
  WT_EXPECT_U64("an empty cursor has nothing left", 0U,
                wt_cursor_remaining(&c));
  WT_EXPECT_TRUE("the remaining view of an empty cursor is NULL",
                 wt_cursor_rest(&c, &len) == NULL);
  ...
  WT_EXPECT_U64("a failed cursor reports nothing remaining", 0U,
                wt_cursor_remaining(&c));
   ...
   * cursor. */
  WT_EXPECT_STATUS("a failed cursor refuses to skip", WT_ERR_TRUNCATED,
                   wt_cursor_skip(&c, 8U));
  c = wt_cursor_init(bytes, sizeof(bytes));
  ...
  WT_EXPECT_STATUS("skipping a NULL cursor is refused",
                   WT_ERR_INVALID_ARGUMENT, wt_cursor_skip(NULL, 1U));
