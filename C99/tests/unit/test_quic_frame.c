...
static size_t encode_ok(const char *label, const wt_quic_frame_t *frame,
...
static wt_status_t round_trip(const char *label, const wt_quic_frame_t *frame,
                              wt_quic_frame_t *out, uint8_t *buffer,
                              size_t capacity) {
...
static void test_rfc9001_crypto_frame(void) {
  ...
  c = wt_cursor_init(WT_RFC9001_CLIENT_INITIAL_PAYLOAD,
  ...
  WT_EXPECT_INT("it is a CRYPTO frame", (long)WT_QUIC_FRAME_KIND_CRYPTO,
                (long)frame.kind);
  WT_EXPECT_U64("with offset 0", WT_RFC9001_CLIENT_INITIAL_CRYPTO_OFFSET,
                frame.as.crypto.offset);
  ...
  WT_EXPECT_BYTES("and the payload is the TLS ClientHello",
                  (const uint8_t *)"\x01\x00\x00\xed\x03\x03",
                  frame.as.crypto.data, 6U);
  /* The frame's own bytes, as printed: type 6, offset 0, a two-byte length of
   * 241. */
  WT_EXPECT_BYTES("the frame's header is the RFC's",
                  (const uint8_t *)"\x06\x00\x40\xf1",
  ...
  while (wt_cursor_remaining(&c) > 0U) {
    WT_EXPECT_STATUS("the next frame parses", WT_OK,
                     wt_quic_frame_decode(&c, &frame, &error));
  ...
  WT_EXPECT_U64("and the rest is PADDING",
                (uint64_t)WT_RFC9001_CLIENT_INITIAL_PADDING_COUNT,
                padding_frames);
  WT_EXPECT_U64("which accounts for every byte of the payload",
                (uint64_t)WT_RFC9001_CLIENT_INITIAL_PAYLOAD_LEN,
                4U + (uint64_t)WT_RFC9001_CLIENT_INITIAL_CRYPTO_LENGTH +
                    padding_frames);
}
...
static void test_frame_round_trips(void) {
  ...
  static const uint8_t path[8] = {0xAAU, 0xBBU, 0xCCU, 0xDDU,
  ...
  WT_EXPECT_STATUS("PADDING round trips", WT_OK,
                   round_trip("PADDING encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_INT("  and decodes as PADDING", (long)WT_QUIC_FRAME_KIND_PADDING,
                (long)decoded.kind);
  frame = wt_quic_frame_make(WT_QUIC_FRAME_KIND_PING);
  WT_EXPECT_STATUS("PING round trips", WT_OK,
                   round_trip("PING encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  frame = wt_quic_frame_make(WT_QUIC_FRAME_KIND_HANDSHAKE_DONE);
  WT_EXPECT_STATUS("HANDSHAKE_DONE round trips", WT_OK,
                   round_trip("HANDSHAKE_DONE encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  WT_EXPECT_INT("  and decodes as itself",
                (long)WT_QUIC_FRAME_KIND_HANDSHAKE_DONE, (long)decoded.kind);
  ...
  WT_EXPECT_STATUS("CRYPTO round trips", WT_OK,
                   round_trip("CRYPTO encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_U64("  with its offset", 4096U, decoded.as.crypto.offset);
  WT_EXPECT_U64("  and length", sizeof(payload),
                (uint64_t)decoded.as.crypto.length);
  WT_EXPECT_BYTES("  and payload", payload, decoded.as.crypto.data,
                  sizeof(payload));
  ...
  WT_EXPECT_STATUS("a zero-length CRYPTO frame round trips", WT_OK,
                   round_trip("a zero-length CRYPTO encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
    ...
    for (has_offset = 0; has_offset < 2; has_offset++) {
      for (has_length = 0; has_length < 2; has_length++) {
        for (fin = 0; fin < 2; fin++) {
          ...
          WT_EXPECT_STATUS("a STREAM flag combination round trips", WT_OK,
                           round_trip("  STREAM encodes", &frame, &decoded,
                                      buffer, sizeof(buffer)));
          WT_EXPECT_INT("  and the offset flag survives", has_offset,
                        decoded.as.stream.has_offset);
          WT_EXPECT_INT("  and the length flag", has_length,
                        decoded.as.stream.has_length);
          ...
          WT_EXPECT_BYTES("  and the payload", payload, decoded.as.stream.data,
                          sizeof(payload));
        }
      }
    }
   ...
   * way, so a reader never has to know. */
  {
    ...
    frame.as.stream.length = sizeof(three_frames);
    {
      ...
      size_t encoded = encode_ok("a STREAM frame with no length encodes", &frame,
                                 buffer, sizeof(buffer));
      c = wt_cursor_init(buffer, encoded);
    }
    WT_EXPECT_STATUS("it parses", WT_OK, wt_quic_frame_decode(&c, &decoded,
                                                              &error));
    WT_EXPECT_U64("taking the rest of the packet as its data",
                  sizeof(three_frames), (uint64_t)decoded.as.stream.length);
  ...
  WT_EXPECT_STATUS("RESET_STREAM round trips", WT_OK,
                   round_trip("RESET_STREAM encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_U64("  with its final size", 1000U,
                decoded.as.reset_stream.final_size);
  ...
  WT_EXPECT_STATUS("STOP_SENDING round trips", WT_OK,
                   round_trip("STOP_SENDING encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_U64("  with its error code", 7U,
                decoded.as.stop_sending.application_error_code);
  ...
  WT_EXPECT_STATUS("RESET_STREAM_AT round trips", WT_OK,
                   round_trip("RESET_STREAM_AT encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  WT_EXPECT_U64("  with its reliable size", 200U,
                decoded.as.reset_stream_at.reliable_size);
  ...
  WT_EXPECT_STATUS("MAX_DATA round trips", WT_OK,
                   round_trip("MAX_DATA encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  ...
  WT_EXPECT_STATUS("MAX_STREAM_DATA round trips", WT_OK,
                   round_trip("MAX_STREAM_DATA encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  ...
  WT_EXPECT_STATUS("MAX_STREAMS round trips", WT_OK,
                   round_trip("MAX_STREAMS encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  ...
  WT_EXPECT_STATUS("the unidirectional form round trips", WT_OK,
                   round_trip("  MAX_STREAMS encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_INT("  with the unidirectional flag",
                (long)WT_QUIC_STREAM_UNIDIRECTIONAL,
                (long)decoded.as.max_streams.direction);
  ...
  WT_EXPECT_STATUS("DATA_BLOCKED round trips", WT_OK,
                   round_trip("DATA_BLOCKED encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  ...
  WT_EXPECT_STATUS("STREAM_DATA_BLOCKED round trips", WT_OK,
                   round_trip("STREAM_DATA_BLOCKED encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  ...
  WT_EXPECT_STATUS("STREAMS_BLOCKED round trips", WT_OK,
                   round_trip("STREAMS_BLOCKED encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  ...
  WT_EXPECT_STATUS("NEW_CONNECTION_ID round trips", WT_OK,
                   round_trip("NEW_CONNECTION_ID encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  WT_EXPECT_U64("  with its connection ID length", sizeof(cid),
                (uint64_t)decoded.as.new_connection_id.connection_id_length);
  WT_EXPECT_BYTES("  and its connection ID", cid,
                  decoded.as.new_connection_id.connection_id, sizeof(cid));
  ...
  WT_EXPECT_STATUS("RETIRE_CONNECTION_ID round trips", WT_OK,
                   round_trip("RETIRE_CONNECTION_ID encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  WT_EXPECT_U64("  with its sequence", 5U,
                decoded.as.retire_connection_id.sequence);
  ...
  WT_EXPECT_STATUS("PATH_CHALLENGE round trips", WT_OK,
                   round_trip("PATH_CHALLENGE encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  WT_EXPECT_BYTES("  with its eight bytes", path,
                  decoded.as.path_challenge.data, 8U);
  ...
  WT_EXPECT_STATUS("PATH_RESPONSE round trips", WT_OK,
                   round_trip("PATH_RESPONSE encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  ...
  WT_EXPECT_STATUS("a transport close round trips", WT_OK,
                   round_trip("  the transport close encodes", &frame, &decoded,
                              buffer, sizeof(buffer)));
  WT_EXPECT_INT("  with its frame type flag", 1,
                decoded.as.connection_close.has_frame_type);
  WT_EXPECT_U64("  and the frame type", WT_QUIC_FRAME_STREAM_BASE,
                decoded.as.connection_close.frame_type);
  WT_EXPECT_BYTES("  and the reason", reason, decoded.as.connection_close.reason,
                  sizeof(reason));
  ...
  frame.as.connection_close.reason = reason;
  frame.as.connection_close.reason_length = sizeof(reason);
  WT_EXPECT_STATUS("an application close round trips", WT_OK,
                   round_trip("  the application close encodes", &frame,
                              &decoded, buffer, sizeof(buffer)));
  WT_EXPECT_INT("  decoding to the application form",
                (long)WT_QUIC_FRAME_KIND_CONNECTION_CLOSE_APPLICATION,
                (long)decoded.kind);
  WT_EXPECT_INT("  with no frame type", 0,
                decoded.as.connection_close.has_frame_type);
  WT_EXPECT_U64("  and its error code", 0x1234U,
                decoded.as.connection_close.error_code);
  ...
  WT_EXPECT_STATUS("a transport close can omit the frame type", WT_OK,
                   round_trip("  it encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_U64("  and reads back as 0", 0U,
                decoded.as.connection_close.frame_type);
  ...
  WT_EXPECT_STATUS("NEW_TOKEN round trips", WT_OK,
                   round_trip("NEW_TOKEN encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_BYTES("  with its token", token, decoded.as.new_token.token,
                  sizeof(token));
  ...
  WT_EXPECT_STATUS("DATAGRAM round trips", WT_OK,
                   round_trip("DATAGRAM encodes", &frame, &decoded, buffer,
                              sizeof(buffer)));
  WT_EXPECT_BYTES("  with its payload", payload, decoded.as.datagram.data,
                  sizeof(payload));
   ...
   * varint, not the other way round. */
  {
    ...
    for (i = 0U; i < sizeof(one_byte) / sizeof(one_byte[0]); i++) {
      frame = wt_quic_frame_make(
          (wt_quic_frame_type_t)((one_byte[i].type == WT_QUIC_FRAME_PADDING)
                                     ? WT_QUIC_FRAME_KIND_PADDING
                                 : (one_byte[i].type == WT_QUIC_FRAME_PING)
                                     ? WT_QUIC_FRAME_KIND_PING
                                     : WT_QUIC_FRAME_KIND_HANDSHAKE_DONE));
      (void)encode_ok("a one-byte frame encodes", &frame, buffer,
                      sizeof(buffer));
      WT_EXPECT_BYTES("  with the RFC's type byte", one_byte[i].bytes, buffer,
                      one_byte[i].length);
    }
    frame = wt_quic_frame_make(WT_QUIC_FRAME_KIND_MAX_DATA);
    frame.as.max_data.maximum = 0x3fU;
    (void)encode_ok("MAX_DATA encodes", &frame, buffer, sizeof(buffer));
    WT_EXPECT_BYTES("  as 0x10 then the varint",
                    (const uint8_t *)"\x10\x3f", buffer, 2U);
    frame.as.max_data.maximum = 0x40U;
    (void)encode_ok("MAX_DATA with a two-byte varint", &frame, buffer,
                    sizeof(buffer));
    WT_EXPECT_BYTES("  as 0x10 0x40 0x40", (const uint8_t *)"\x10\x40\x40",
                    buffer, 3U);
  }
   ...
   * reads first. */
  {
    ...
    for (i = 0U; i < sizeof(types) / sizeof(types[0]); i++) {
      ...
      WT_EXPECT_U64("  with the type byte its flags imply", types[i].type,
                    (uint64_t)buffer[0]);
    }
  }
}
...
static void test_frame_refusals(void) {
   ...
   * or the extension range. */
  {
    ...
    WT_EXPECT_U64("  as a frame encoding error", WT_QUIC_FRAME_ENCODING_ERROR,
                  error);
    WT_EXPECT_U64("  and nothing was consumed beyond the type", 1U,
                  (uint64_t)c.offset);
  }
  ...
  /* Truncated bodies: each frame kind with its last byte missing. */
  {
    static const struct {
      ...
      size_t length;
        ...
        {"PATH_CHALLENGE with seven bytes", {0x1aU, 1U, 2U, 3U, 4U, 5U, 6U, 7U},
         8U},
  ...
  /* Each field whose value the RFC gives a rule. */
  {
    ...
    /* A retire_prior_to above the sequence would retire the ID being issued. */
    {
      ...
      error = 0U;
      WT_EXPECT_STATUS("retire_prior_to above the sequence is refused",
                       WT_ERR_PROTOCOL, wt_quic_frame_decode(&c, &decoded,
                                                             &error));
    }
    /* RESET_STREAM_AT with a reliable size above the final size. */
    {
      ...
      error = 0U;
      WT_EXPECT_STATUS("a reliable size above the final size is refused",
                       WT_ERR_PROTOCOL,
                       wt_quic_frame_decode(&c, &decoded, &error));
      WT_EXPECT_U64("  as a frame encoding error",
   ...
   * the end. The varint for 2^62 - 1 is eight bytes of 0xff after the prefix. */
  {
    ...
    error = 0U;
    WT_EXPECT_STATUS("a length that cannot be a size is refused",
                     WT_ERR_TRUNCATED, wt_quic_frame_decode(&c, &decoded,
                                                            &error));
  }
   ...
   * accept. */
  {
    ...
    WT_EXPECT_STATUS("encoding a 21-byte connection ID is refused",
    ...
    WT_EXPECT_STATUS("encoding an empty NEW_TOKEN is refused",
                     WT_ERR_INVALID_ARGUMENT, wt_quic_frame_encode(&w, &frame));
    ...
    WT_EXPECT_STATUS("encoding into a buffer that is too small is a limit",
                     WT_ERR_LIMIT, wt_quic_frame_encode(&w, &frame));
...
static void test_frame_walk(void) {
...

  {
    uint64_t counts[32];
    size_t i;
    for (i = 0U; i < 32U; i++) counts[i] = 0U;
    WT_EXPECT_STATUS("the walk visits every frame", WT_OK,
                     wt_quic_frames_decode(buffer, wt_writer_offset(&w),
                                           count_frames, counts, &error));
  ...
  {
    uint64_t seen = 0U;
    status = wt_quic_frames_decode(buffer, wt_writer_offset(&w), stop_at_ping,
                                   &seen, &error);
  ...
  /* A malformed frame in the middle stops the walk and reports the code. */
  {
    ...
    WT_EXPECT_STATUS("a malformed frame stops the walk", WT_ERR_PROTOCOL,
                     wt_quic_frames_decode(bad, 3U, count_frames, counts,
                                           &error));
    WT_EXPECT_U64("  with the frame encoding error",
                  WT_QUIC_FRAME_ENCODING_ERROR, error);
    WT_EXPECT_U64("  and one frame was visited", 1U,
                  counts[WT_QUIC_FRAME_KIND_PING]);
  }
  ...
  WT_EXPECT_STATUS("a NULL visitor checks without visiting", WT_OK,
                   wt_quic_frames_decode(buffer, wt_writer_offset(&w), NULL,
                                         NULL, &error));
  WT_EXPECT_STATUS("a NULL buffer with a length is refused",
...
static void test_ack_ranges(void) {
   ...
   * structure the parser keeps as bytes. */
  {
     ...
     * them. */
    {
      ...
      encoded = encode_ok("an ACK with three ranges encodes", &holder, buffer,
                          sizeof(buffer));
   ...
   * and the accessor decodes them one at a time. */
  {
    ...
    WT_EXPECT_STATUS("the hand-built ACK parses", WT_OK,
                     wt_quic_frame_decode(&c, &decoded, &error));
    WT_EXPECT_INT("it is an ACK", (long)WT_QUIC_FRAME_KIND_ACK,
                  (long)decoded.kind);
    ...
    for (i = 0U; i < 3U; i++) {
      WT_EXPECT_STATUS("the range decodes", WT_OK,
                       wt_quic_frame_ack_range_at(&decoded, i, &range));
      WT_EXPECT_U64("  gap", expected_gap[i], range.gap);
      WT_EXPECT_U64("  length", expected_length[i], range.length);
    }
    WT_EXPECT_STATUS("an index past the end is refused",
                     WT_ERR_INVALID_ARGUMENT,
   ...
   * the end. */
  {
    static const uint8_t short_ack[] = {0x02U, 0x40U, 0x64U, 0x07U, 0x05U,
                                        0x03U, 0x01U, 0x03U};
    wt_cursor_t c = wt_cursor_init(short_ack, sizeof(short_ack));
    WT_EXPECT_STATUS("an ACK with missing ranges is truncated",
                     WT_ERR_TRUNCATED, wt_quic_frame_decode(&c, &decoded,
                                                            &error));
  }
   ...
   * allocate or loop: it walks the ranges it has and refuses. */
  {
    ...
    static const uint8_t many[15] = {0x02U, 0x40U, 0x64U, 0x07U, 0xffU, 0xffU,
                                     0xffU, 0xffU, 0xffU, 0xffU, 0xffU, 0xffU,
                                     0x03U, 0x01U, 0x03U};
    wt_cursor_t c = wt_cursor_init(many, sizeof(many));
    WT_EXPECT_STATUS("an ACK claiming many ranges is refused without walking",
                     WT_ERR_TRUNCATED, wt_quic_frame_decode(&c, &decoded,
                                                            &error));
  }
  ...
  {
    static const uint8_t ecn[] = {0x03U, 0x40U, 0x64U, 0x07U, 0x00U,
                                  0x03U, 0x01U, 0x02U, 0x00U};
    wt_cursor_t c = wt_cursor_init(ecn, sizeof(ecn));
    WT_EXPECT_STATUS("an ACK_ECN parses", WT_OK,
                     wt_quic_frame_decode(&c, &decoded, &error));
  ...
  frame = wt_quic_frame_make(WT_QUIC_FRAME_KIND_PING);
  WT_EXPECT_STATUS("the range accessor refuses a non-ACK",
                   WT_ERR_INVALID_ARGUMENT,
                   wt_quic_frame_ack_range_at(&frame, 0U, &range));
  WT_EXPECT_STATUS("the range accessor refuses a NULL frame",
                   WT_ERR_INVALID_ARGUMENT,
                   wt_quic_frame_ack_range_at(NULL, 0U, &range));
}

static void test_frame_names(void) {
  WT_EXPECT_STR("padding", "padding",
                wt_quic_frame_kind_name(WT_QUIC_FRAME_KIND_PADDING));
  WT_EXPECT_STR("stream", "stream",
                wt_quic_frame_kind_name(WT_QUIC_FRAME_KIND_STREAM));
  WT_EXPECT_STR("connection close", "connection-close",
                wt_quic_frame_kind_name(
  ...
  WT_EXPECT_STR("datagram", "datagram",
                wt_quic_frame_kind_name(WT_QUIC_FRAME_KIND_DATAGRAM));
  WT_EXPECT_STR("an unknown kind", "unknown",
                wt_quic_frame_kind_name((wt_quic_frame_type_t)999));
}
