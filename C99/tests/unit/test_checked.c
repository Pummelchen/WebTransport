...
int main(void) {
  ...
  WT_EXPECT_STATUS("the largest sum that fits", WT_OK,
                   wt_checked_add_size(SIZE_MAX - 1U, 1U, &size_result));
  WT_EXPECT_U64("the largest sum that fits is SIZE_MAX", SIZE_MAX, size_result);
  WT_EXPECT_STATUS("SIZE_MAX + 1 is refused", WT_ERR_OVERFLOW,
                   wt_checked_add_size(SIZE_MAX, 1U, &size_result));
  WT_EXPECT_STATUS("SIZE_MAX + SIZE_MAX is refused", WT_ERR_OVERFLOW,
                   wt_checked_add_size(SIZE_MAX, SIZE_MAX, &size_result));
  WT_EXPECT_STATUS("addition refuses a NULL output", WT_ERR_INVALID_ARGUMENT,
                   wt_checked_add_size(1U, 1U, NULL));
  ...
  WT_EXPECT_STATUS("u64 addition refuses the wrap", WT_ERR_OVERFLOW,
                   wt_checked_add_u64(UINT64_MAX, 1U, &u64_result));
  ...
  WT_EXPECT_STATUS("u64 subtraction refuses the wrap", WT_ERR_OVERFLOW,
                   wt_checked_sub_u64(0U, 1U, &u64_result));
  ...
  WT_EXPECT_STATUS("anything times zero is zero", WT_OK,
                   wt_checked_mul_size(SIZE_MAX, 0U, &size_result));
  WT_EXPECT_U64("times zero = 0", 0U, size_result);
  WT_EXPECT_STATUS("zero times anything is zero", WT_OK,
                   wt_checked_mul_size(0U, SIZE_MAX, &size_result));
  ...
  WT_EXPECT_STATUS("SIZE_MAX * 2 is refused", WT_ERR_OVERFLOW,
                   wt_checked_mul_size(SIZE_MAX, 2U, &size_result));
  WT_EXPECT_STATUS("2 * SIZE_MAX is refused", WT_ERR_OVERFLOW,
                   wt_checked_mul_size(2U, SIZE_MAX, &size_result));
  /* The pre-allocation overflow: a count that wraps to a small number. */
  WT_EXPECT_STATUS("a count that would wrap is refused", WT_ERR_OVERFLOW,
                   wt_checked_mul_size((SIZE_MAX / 2U) + 1U, 2U, &size_result));
  WT_EXPECT_STATUS("multiplication refuses a NULL output", WT_ERR_INVALID_ARGUMENT,
                   wt_checked_mul_size(2U, 2U, NULL));
  WT_EXPECT_STATUS("u64 multiplication refuses the wrap", WT_ERR_OVERFLOW,
                   wt_checked_mul_u64(UINT64_MAX, 2U, &u64_result));
  ...
  WT_EXPECT_STATUS("UINT32_MAX + 1 does not fit", WT_ERR_OVERFLOW,
                   wt_checked_narrow_u64_to_u32((uint64_t)UINT32_MAX + 1U, &u32_result));
  WT_EXPECT_STATUS("narrowing refuses a NULL output", WT_ERR_INVALID_ARGUMENT,
                   wt_checked_narrow_u64_to_u32(1U, NULL));
  WT_EXPECT_STATUS("a u64 that fits in size_t", WT_OK,
                   wt_checked_narrow_u64_to_size(1U, &size_result));
  ...
  WT_EXPECT_STATUS("size_t to u32 refuses a NULL output", WT_ERR_INVALID_ARGUMENT,
