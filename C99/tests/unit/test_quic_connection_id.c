...
int main(void) {
  ...
  WT_EXPECT_STATUS("the store initialises", WT_OK,
                   wt_quic_connection_ids_init(&store, 4U,
                                               WT_QUIC_CONNECTION_ID_LIMIT));
  WT_EXPECT_U64("with the peer's limit", 4U, store.peer_limit);
  WT_EXPECT_U64("and this endpoint's", WT_QUIC_CONNECTION_ID_LIMIT,
                store.local_limit);
  WT_EXPECT_U64("nothing is active", 0U,
                (uint64_t)wt_quic_connection_ids_active(&store));
  WT_EXPECT_U64("and four are issueable", 4U,
                wt_quic_connection_ids_issueable(&store));
  ...
  for (i = 0U; i < 4U; i++) {
    ...
    WT_EXPECT_STATUS("an issued ID is added", WT_OK,
                     wt_quic_connection_ids_add_issued(&store, id, 8U,
                                                       &sequence));
    WT_EXPECT_U64("with the next sequence", (uint64_t)i, sequence);
  }
  WT_EXPECT_U64("four are active", 4U,
                (uint64_t)wt_quic_connection_ids_active(&store));
  WT_EXPECT_U64("and none are issueable", 0U,
                wt_quic_connection_ids_issueable(&store));
  fill(id, 8U, 9U);
  ...
  WT_EXPECT_U64("and the count did not move", 4U,
                (uint64_t)wt_quic_connection_ids_active(&store));

  /* Retiring one frees a slot in the peer's limit, because section 5.1.1 counts
   * what the endpoint is currently providing. */
  WT_EXPECT_STATUS("retiring sequence 0", WT_OK,
                   wt_quic_connection_ids_retire(&store, 0U, &error));
  WT_EXPECT_U64("leaves three active", 3U,
                (uint64_t)wt_quic_connection_ids_active(&store));
  WT_EXPECT_U64("and one issueable", 1U,
                wt_quic_connection_ids_issueable(&store));
  fill(id, 8U, 9U);
   ...
   * issued, and a retired ID is reported as retired rather than as unknown. */
  {
    ...
    WT_EXPECT_STATUS("an issued ID is found", WT_OK,
                     wt_quic_connection_ids_find(&store, issued, 8U, &index,
                                                 &retired));
    ...
    WT_EXPECT_STATUS("a zero-length ID is not found here", WT_ERR_CLOSED,
                     wt_quic_connection_ids_find(&store, NULL, 0U, &index,
                                                 &retired));
  }
  ...
  /* A peer's NEW_CONNECTION_ID, including retire_prior_to. */
  {
    ...
    WT_EXPECT_STATUS("a peer store initialises", WT_OK,
                     wt_quic_connection_ids_init(&peer, 8U, 4U));

    fill(id, 6U, 0x10U);
    WT_EXPECT_STATUS("a peer ID is added", WT_OK,
                     wt_quic_connection_ids_add_peer(&peer, 3U, 0U, id, 6U,
                                                     &error));
    WT_EXPECT_U64("it is active", 1U,
    ...
    WT_EXPECT_STATUS("a peer ID with retire_prior_to", WT_OK,
                     wt_quic_connection_ids_add_peer(&peer, 5U, 4U, id, 6U,
                                                     &error));
    WT_EXPECT_U64("retires what is below it", 1U,
                  (uint64_t)wt_quic_connection_ids_active(&peer));
    ...
    WT_EXPECT_U64("and reports the connection ID limit error",
                  WT_QUIC_CONNECTION_ID_LIMIT_ERROR, error);
    ...
    WT_EXPECT_STATUS("retire_prior_to above the sequence is refused",
                     WT_ERR_PROTOCOL,
                     wt_quic_connection_ids_add_peer(&peer, 1U, 2U, id, 6U,
                                                     &error));
    ...
    /* A connection ID above twenty bytes. */
    {
      ...
      WT_EXPECT_STATUS("a 21-byte connection ID is refused", WT_ERR_PROTOCOL,
                       wt_quic_connection_ids_add_peer(&limit, 0U, 0U, long_id,
                                                       sizeof(long_id), &error));
     ...
     * array: the bound is this implementation's, not the peer's. */
    {
      ...
      for (i = 0U; i < 64U; i++) {
        ...
        if (wt_quic_connection_ids_add_peer(&big, i, 0U, id, 4U, &error) !=
            WT_OK) {
          break;
        }
        added++;
      }
      WT_EXPECT_U64("the array bound stops it", WT_QUIC_CONNECTION_ID_LIMIT,
                    (uint64_t)added);
    }
  }
  ...
  WT_EXPECT_STATUS("adding an over-long issued ID is refused",
  ...
  WT_EXPECT_STATUS("adding NULL bytes is refused", WT_ERR_INVALID_ARGUMENT,
                   wt_quic_connection_ids_add_issued(&store, NULL, 4U,
                                                     &sequence));
  WT_EXPECT_STATUS("retiring on a NULL store is refused",
                   WT_ERR_INVALID_ARGUMENT,
                   wt_quic_connection_ids_retire(NULL, 0U, &error));
  WT_EXPECT_STATUS("finding on a NULL store is refused",
                   WT_ERR_INVALID_ARGUMENT,
                   wt_quic_connection_ids_find(NULL, id, 4U, &index, &retired));
  WT_EXPECT_STATUS("finding with a NULL output is refused",
                   WT_ERR_INVALID_ARGUMENT,
  ...
  WT_EXPECT_U64("and nothing issueable", 0U,
                wt_quic_connection_ids_issueable(NULL));
   ...
   * 5.1 says an endpoint may use one, and it must be storable and findable. */
  {
    ...
    WT_EXPECT_STATUS("a zero-length issued ID is added", WT_OK,
                     wt_quic_connection_ids_add_issued(&zero, NULL, 0U,
                                                       &sequence));
    WT_EXPECT_STATUS("and is found by its empty bytes", WT_OK,
                     wt_quic_connection_ids_find(&zero, NULL, 0U, &index,
                                                 &retired));
