...
int main(void) {
  ...
  WT_EXPECT_U64("which is the range's own last codepoint", WT_WEBTRANSPORT_APPLICATION_ERROR_LAST,
                wt_webtransport_error_to_http3(UINT32_C(0xffffffff)));
  WT_EXPECT_U64("and the range's first is what zero maps to",
                WT_WEBTRANSPORT_APPLICATION_ERROR_FIRST, wt_webtransport_error_to_http3(0U));
  ...
  {
    static const uint32_t k_samples[] = {0U,      1U,          0x1dU,       0x1eU,
                                         0x1fU,   0x3cU,       0x3dU,       0x3eU,
                                         0x1000U, 0x12345678U, 0xfffffffeU, 0xffffffffU};
  ...
  /* And back again. */
  WT_EXPECT_OK(
      "the first codepoint carries application error zero",
  ...
  {
    static const uint32_t k_samples[] = {0U,    1U,    0x1dU,       0x1eU,
                                         0x1fU, 0x3cU, 0x12345678U, 0xffffffffU};
    for (i = 0U; i < sizeof(k_samples) / sizeof(k_samples[0]); i++) {
  ...
  WT_EXPECT_STATUS(
      "a codepoint below the range is refused", WT_ERR_INVALID_ARGUMENT,
      wt_webtransport_error_from_http3(WT_WEBTRANSPORT_APPLICATION_ERROR_FIRST - 1U, &round_trip));
  WT_EXPECT_STATUS(
      "and one above it", WT_ERR_INVALID_ARGUMENT,
      wt_webtransport_error_from_http3(WT_WEBTRANSPORT_APPLICATION_ERROR_LAST + 1U, &round_trip));
  {
    uint64_t reserved =
        UINT64_C(0x1f) *
            ((WT_WEBTRANSPORT_APPLICATION_ERROR_FIRST - UINT64_C(0x21)) / UINT64_C(0x1f)) +
        UINT64_C(0x21);
    while (reserved < WT_WEBTRANSPORT_APPLICATION_ERROR_FIRST)
      reserved += UINT64_C(0x1f);
  ...
  WT_EXPECT_U64("and the name this tree used before still means it",
                WT_WEBTRANSPORT_ERROR_FLOW_CONTROL, WT_WEBTRANSPORT_FLOW_CONTROL_ERROR);
  ...
  {
    static const uint64_t k_protocol_codes[] = {UINT64_C(0x3994bd84), UINT64_C(0x170d7b68),
                                                UINT64_C(0x045d4487), UINT64_C(0x0817b3dd),
                                                UINT64_C(0x212c0d48)};
