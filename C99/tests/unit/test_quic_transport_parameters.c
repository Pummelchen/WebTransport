...
int main(void) {
  ...
  WT_EXPECT_STATUS(
      "initial_max_data", WT_OK,
      wt_quic_transport_parameters_add_integer(&params, WT_QUIC_TP_INITIAL_MAX_DATA, 1048576U));
  WT_EXPECT_STATUS(
      "max_idle_timeout", WT_OK,
      wt_quic_transport_parameters_add_integer(&params, WT_QUIC_TP_MAX_IDLE_TIMEOUT, 30000U));
  WT_EXPECT_STATUS(
      "max_udp_payload_size", WT_OK,
      wt_quic_transport_parameters_add_integer(&params, WT_QUIC_TP_MAX_UDP_PAYLOAD_SIZE, 1472U));
  WT_EXPECT_STATUS(
      "active_connection_id_limit", WT_OK,
  ...
  WT_EXPECT_U64("the first is the lowest identifier", WT_QUIC_TP_MAX_IDLE_TIMEOUT,
                params.entries[0].id);

  /* A duplicate is refused when adding and again when encoding. */
  WT_EXPECT_STATUS(
      "a duplicate identifier is refused", WT_ERR_INVALID_ARGUMENT,
      wt_quic_transport_parameters_add_integer(&params, WT_QUIC_TP_MAX_IDLE_TIMEOUT, 1U));

  /* The values are found by identifier. */
  WT_EXPECT_STATUS(
      "initial_max_data reads back", WT_OK,
      wt_quic_transport_parameters_integer(&params, WT_QUIC_TP_INITIAL_MAX_DATA, &value));
  WT_EXPECT_U64("with its value", 1048576U, value);
  WT_EXPECT_STATUS(
      "max_udp_payload_size reads back", WT_OK,
      wt_quic_transport_parameters_integer(&params, WT_QUIC_TP_MAX_UDP_PAYLOAD_SIZE, &value));
  WT_EXPECT_U64("with its value", 1472U, value);
  /* Absent is not zero. */
  WT_EXPECT_STATUS(
      "an absent parameter is not zero", WT_ERR_INVALID_ARGUMENT,
      wt_quic_transport_parameters_integer(&params, WT_QUIC_TP_STATELESS_RESET_TOKEN, &value));
  found = NULL;
  WT_EXPECT_STATUS(
      "a present parameter is found", WT_OK,
      wt_quic_transport_parameters_get(&params, WT_QUIC_TP_MAX_IDLE_TIMEOUT, &found, &length));
  ...
  WT_EXPECT_STATUS(
      "an absent parameter is not found", WT_ERR_INVALID_ARGUMENT,
      wt_quic_transport_parameters_get(&params, WT_QUIC_TP_STATELESS_RESET_TOKEN, &found, &length));
   ...
   * value view may be NULL, which is not the same as being absent. */
  {
    ...
    (void)wt_quic_transport_parameters_add_bytes(&empty, WT_QUIC_TP_DISABLE_ACTIVE_MIGRATION, NULL,
                                                 0U);
    found = (const uint8_t *)"x";
    length = 99U;
    WT_EXPECT_STATUS("an empty-valued parameter is present", WT_OK,
                     wt_quic_transport_parameters_get(&empty, WT_QUIC_TP_DISABLE_ACTIVE_MIGRATION,
                                                      &found, &length));
  ...
  /* Encode and parse back. */
  {
    ...
    WT_EXPECT_STATUS("the list encodes", WT_OK, wt_quic_transport_parameters_encode(&w, &params));
    WT_EXPECT_TRUE("to something", wt_writer_offset(&w) > 0U);
    WT_EXPECT_STATUS(
        "and parses back", WT_OK,
        wt_quic_transport_parameters_decode(buffer, wt_writer_offset(&w), &parsed, &error));
    WT_EXPECT_U64("with the same count", (uint64_t)params.count, (uint64_t)parsed.count);
    WT_EXPECT_INT("and sorted", 1, parsed.sorted);
    WT_EXPECT_STATUS(
        "initial_max_data survives", WT_OK,
        wt_quic_transport_parameters_integer(&parsed, WT_QUIC_TP_INITIAL_MAX_DATA, &value));
    WT_EXPECT_U64("with its value", 1048576U, value);
    WT_EXPECT_STATUS(
        "max_idle_timeout survives", WT_OK,
        wt_quic_transport_parameters_integer(&parsed, WT_QUIC_TP_MAX_IDLE_TIMEOUT, &value));
  ...
  /* An unknown parameter is kept, not refused: RFC 9000 section 7.4.2. */
  {
    ...
    wt_quic_transport_parameters_t parsed;
    WT_EXPECT_STATUS(
        "an unknown parameter parses", WT_OK,
        wt_quic_transport_parameters_decode(unknown, sizeof(unknown), &parsed, &error));
    WT_EXPECT_U64("and is kept", 1U, (uint64_t)parsed.count);
    WT_EXPECT_U64("under its own identifier", 0x21U, parsed.entries[0].id);
    WT_EXPECT_U64("with its value", 2U, (uint64_t)parsed.entries[0].length);
    WT_EXPECT_STR("with a name of unknown", "unknown", wt_quic_transport_parameter_name(0x21U));
  }

  /* A duplicate identifier is a TRANSPORT_PARAMETER_ERROR (section 7.4). */
  {
    static const uint8_t duplicate[] = {0x01U, 0x01U, 0x0AU, 0x01U, 0x01U, 0x0BU};
    wt_quic_transport_parameters_t parsed;
    error = 0U;
    WT_EXPECT_STATUS(
        "a duplicate parameter is refused", WT_ERR_PROTOCOL,
        wt_quic_transport_parameters_decode(duplicate, sizeof(duplicate), &parsed, &error));
    WT_EXPECT_U64("  as a transport parameter error", WT_QUIC_TRANSPORT_PARAMETER_ERROR, error);
  }

  /* A non-minimal length is refused: the same rule as a frame type. */
  {
    static const uint8_t padded_length[] = {0x01U, 0x80U, 0x00U, 0x00U, 0x01U, 0x0AU};
    wt_quic_transport_parameters_t parsed;
    error = 0U;
    WT_EXPECT_STATUS(
        "a non-minimal length is refused", WT_ERR_PROTOCOL,
        wt_quic_transport_parameters_decode(padded_length, sizeof(padded_length), &parsed, &error));
  }
  ...
  /* Truncation, at the identifier, the length and the value. */
  {
    ...
    WT_EXPECT_STATUS("a truncated identifier is refused", WT_ERR_TRUNCATED,
                     wt_quic_transport_parameters_decode(cut_id, sizeof(cut_id), &parsed, &error));
    WT_EXPECT_STATUS(
        "a truncated length is refused", WT_ERR_TRUNCATED,
        wt_quic_transport_parameters_decode(cut_length, sizeof(cut_length), &parsed, &error));
    WT_EXPECT_STATUS(
        "a truncated value is refused", WT_ERR_TRUNCATED,
        wt_quic_transport_parameters_decode(cut_value, sizeof(cut_value), &parsed, &error));
    WT_EXPECT_STATUS(
        "a NULL list is refused", WT_ERR_INVALID_ARGUMENT,
        wt_quic_transport_parameters_decode(cut_value, sizeof(cut_value), NULL, &error));
  }
   ...
   * a client's first flight does when everything has a default. */
  {
    ...
    WT_EXPECT_STATUS("an empty list parses", WT_OK,
                     wt_quic_transport_parameters_decode(NULL, 0U, &parsed, &error));
  ...
  {
    static const uint8_t unsorted[] = {0x04U, 0x01U, 0x0AU, 0x01U, 0x01U, 0x0BU};
    wt_quic_transport_parameters_t parsed;
    WT_EXPECT_STATUS(
        "an unsorted list parses", WT_OK,
        wt_quic_transport_parameters_decode(unsorted, sizeof(unsorted), &parsed, &error));
    WT_EXPECT_INT("and is marked unsorted", 0, parsed.sorted);
    WT_EXPECT_STATUS(
        "and its first parameter is found", WT_OK,
        wt_quic_transport_parameters_integer(&parsed, WT_QUIC_TP_INITIAL_MAX_DATA, &value));
    WT_EXPECT_U64("with its value", 10U, value);
    WT_EXPECT_STATUS(
        "and its second", WT_OK,
        wt_quic_transport_parameters_integer(&parsed, WT_QUIC_TP_MAX_IDLE_TIMEOUT, &value));
    ...
    /* A duplicate in an unsorted list is still found. */
    {
      ...
      static const uint8_t unsorted_duplicate[] = {0x04U, 0x01U, 0x0AU, 0x01U, 0x01U,
                                                   0x0BU, 0x04U, 0x01U, 0x0CU};
      error = 0U;
      WT_EXPECT_STATUS("a duplicate in an unsorted list is refused", WT_ERR_PROTOCOL,
                       wt_quic_transport_parameters_decode(
                           unsorted_duplicate, sizeof(unsorted_duplicate), &parsed, &error));
    }
  }
   ...
   * caller that wanted an integer asks. */
  {
    ...
    wt_quic_transport_parameters_t parsed;
    WT_EXPECT_STATUS(
        "a two-byte value is well framed", WT_OK,
        wt_quic_transport_parameters_decode(two_varints, sizeof(two_varints), &parsed, &error));
    WT_EXPECT_STATUS(
        "but is not one integer", WT_ERR_TRUNCATED,
        wt_quic_transport_parameters_integer(&parsed, WT_QUIC_TP_INITIAL_MAX_DATA, &value));
    WT_EXPECT_STATUS(
        "an empty value is well framed", WT_OK,
        wt_quic_transport_parameters_decode(empty_value, sizeof(empty_value), &parsed, &error));
    WT_EXPECT_STATUS(
        "but is not an integer either", WT_ERR_TRUNCATED,
        wt_quic_transport_parameters_integer(&parsed, WT_QUIC_TP_INITIAL_MAX_DATA, &value));
  }
  ...
  /* The section 18.2 value rules, each at its boundary. */
  {
    ...
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_MAX_UDP_PAYLOAD_SIZE, 1199U);
    error = 0U;
    offender = 0U;
    WT_EXPECT_STATUS("max_udp_payload_size of 1199 is refused", WT_ERR_PROTOCOL,
                     wt_quic_transport_parameters_check(&check, 0, &error, &offender));
    WT_EXPECT_U64("  as a transport parameter error", WT_QUIC_TRANSPORT_PARAMETER_ERROR, error);
    WT_EXPECT_U64("  naming the parameter", WT_QUIC_TP_MAX_UDP_PAYLOAD_SIZE, offender);
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_MAX_UDP_PAYLOAD_SIZE, 1200U);
   ...
   * a negotiation nobody can rely on. The empty form is the only one that means "I support this extension". */
    ...
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_ACK_DELAY_EXPONENT, 20U);
    WT_EXPECT_STATUS("ack_delay_exponent of 20 is accepted", WT_OK,
                     wt_quic_transport_parameters_check(&check, 0, &error, &offender));
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_ACK_DELAY_EXPONENT, 21U);
    WT_EXPECT_STATUS("ack_delay_exponent of 21 is refused", WT_ERR_PROTOCOL,
                     wt_quic_transport_parameters_check(&check, 0, &error, &offender));

    /* max_ack_delay of 2^14 - 1 is fine, 2^14 is not. */
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_MAX_ACK_DELAY,
                                                   (UINT64_C(1) << 14) - 1U);
    ...
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_ACTIVE_CONNECTION_ID_LIMIT,
                                                   1U);
    WT_EXPECT_STATUS("active_connection_id_limit of 1 is refused", WT_ERR_PROTOCOL,
                     wt_quic_transport_parameters_check(&check, 0, &error, &offender));
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_ACTIVE_CONNECTION_ID_LIMIT,
                                                   2U);
    ...
    /* A stateless reset token that is not sixteen bytes. */
    {
      ...
      wt_quic_transport_parameters_init(&check);
      (void)wt_quic_transport_parameters_add_bytes(&check, WT_QUIC_TP_STATELESS_RESET_TOKEN,
                                                   short_token, sizeof(short_token));
      WT_EXPECT_STATUS("a 15-byte stateless reset token is refused", WT_ERR_PROTOCOL,
                       wt_quic_transport_parameters_check(&check, 0, &error, &offender));
      wt_quic_transport_parameters_init(&check);
      (void)wt_quic_transport_parameters_add_bytes(&check, WT_QUIC_TP_STATELESS_RESET_TOKEN,
                                                   exactly_sixteen, sizeof(exactly_sixteen));
    ...
    /* original_destination_connection_id is never empty and never above 20. */
    {
      ...
      WT_EXPECT_STATUS("an empty original destination ID is refused", WT_ERR_PROTOCOL,
                       wt_quic_transport_parameters_check(&check, 0, &error, &offender));
      WT_EXPECT_U64("  naming the parameter", WT_QUIC_TP_ORIGINAL_DESTINATION_CONNECTION_ID,
                    offender);
      wt_quic_transport_parameters_init(&check);
      (void)wt_quic_transport_parameters_add_bytes(
          &check, WT_QUIC_TP_ORIGINAL_DESTINATION_CONNECTION_ID, twenty_one, sizeof(twenty_one));
      WT_EXPECT_STATUS("a 21-byte original destination ID is refused", WT_ERR_PROTOCOL,
                       wt_quic_transport_parameters_check(&check, 0, &error, &offender));
      wt_quic_transport_parameters_init(&check);
      (void)wt_quic_transport_parameters_add_bytes(
          &check, WT_QUIC_TP_ORIGINAL_DESTINATION_CONNECTION_ID, eight, sizeof(eight));
     ...
     * zero-length connection ID -- but not above twenty. */
    {
      static const uint8_t twenty_one[21] = {0};
      wt_quic_transport_parameters_init(&check);
      (void)wt_quic_transport_parameters_add_bytes(&check, WT_QUIC_TP_INITIAL_SOURCE_CONNECTION_ID,
                                                   NULL, 0U);
      WT_EXPECT_STATUS("an empty initial source connection ID is accepted", WT_OK,
                       wt_quic_transport_parameters_check(&check, 0, &error, &offender));
      wt_quic_transport_parameters_init(&check);
      (void)wt_quic_transport_parameters_add_bytes(&check, WT_QUIC_TP_RETRY_SOURCE_CONNECTION_ID,
                                                   twenty_one, sizeof(twenty_one));
      WT_EXPECT_STATUS("a 21-byte retry source connection ID is refused", WT_ERR_PROTOCOL,
                       wt_quic_transport_parameters_check(&check, 0, &error, &offender));
    }
    ...
    wt_quic_transport_parameters_init(&check);
    (void)wt_quic_transport_parameters_add_integer(&check, WT_QUIC_TP_MAX_DATAGRAM_FRAME_SIZE, 0U);
   ...
   * holds is a protocol error rather than a silent truncation. */
  {
    ...
    for (i = 0U; i < WT_QUIC_MAX_TRANSPORT_PARAMETERS; i++) {
      WT_EXPECT_STATUS(
          "a parameter fits", WT_OK,
          wt_quic_transport_parameters_add_integer(&full, 0x100U + (uint64_t)i, (uint64_t)i));
    }
    WT_EXPECT_U64("the list is full", WT_QUIC_MAX_TRANSPORT_PARAMETERS, (uint64_t)full.count);
    ...
    /* One more than the bound on the wire: 33 single-byte parameters. */
    {
      ...
      WT_EXPECT_STATUS("33 parameters are refused", WT_ERR_PROTOCOL,
                       wt_quic_transport_parameters_decode(encoded, at, &parsed, &error));
      WT_EXPECT_U64("  as a transport parameter error", WT_QUIC_TRANSPORT_PARAMETER_ERROR, error);
    }
  }
  ...
  /* Encoding refuses a duplicate, and a NULL argument. */
  {
    ...
    (void)wt_quic_transport_parameters_add_integer(&duplicate, WT_QUIC_TP_MAX_IDLE_TIMEOUT, 1U);
    ...
    WT_EXPECT_STATUS("an integer lookup with a NULL output is refused", WT_ERR_INVALID_ARGUMENT,
                     wt_quic_transport_parameters_integer(&params, 0U, NULL));
    WT_EXPECT_STATUS("a check with a NULL list is refused", WT_ERR_INVALID_ARGUMENT,
                     wt_quic_transport_parameters_check(NULL, 0, &error, &offender));
    WT_EXPECT_STATUS("adding to a NULL list is refused", WT_ERR_INVALID_ARGUMENT,
    ...
    WT_EXPECT_STATUS("adding NULL bytes is refused", WT_ERR_INVALID_ARGUMENT,
                     wt_quic_transport_parameters_add_bytes(&params, 0x900U, NULL, 4U));
  }
  ...
  WT_EXPECT_STR("max_udp_payload_size", "max_udp_payload_size",
                wt_quic_transport_parameter_name(WT_QUIC_TP_MAX_UDP_PAYLOAD_SIZE));
  WT_EXPECT_STR("active_connection_id_limit", "active_connection_id_limit",
                wt_quic_transport_parameter_name(WT_QUIC_TP_ACTIVE_CONNECTION_ID_LIMIT));
  WT_EXPECT_STR("max_datagram_frame_size", "max_datagram_frame_size",
                wt_quic_transport_parameter_name(WT_QUIC_TP_MAX_DATAGRAM_FRAME_SIZE));
  WT_EXPECT_STR("an unassigned identifier", "unknown", wt_quic_transport_parameter_name(0x4321U));
