...
void wt_quic_stream_table_init(wt_quic_stream_table_t *table) {
  ...
  /* Zeroed rather than field by field: `used` is what says a slot is live, so clearing it is what makes
   * the table empty, and the stream structures behind it are then indeterminate but unreachable. */
...
wt_status_t wt_quic_stream_table_open(wt_quic_stream_table_t *table, uint64_t stream_id,
  ...
  /* The limit is the OPENER's: a peer-initiated stream costs the peer one of its allowance, not this
   * endpoint one of its own (RFC 9000 section 4.6). */
  ...
  /* A new stream has no flow control credit at all: the caller sets the two limits from the transport
   * parameters it and its peer sent, because the table is not the layer that knows them. */
...
wt_status_t wt_quic_stream_table_close(wt_quic_stream_table_t *table, uint64_t stream_id) {
  ...
  /* A stream is only forgotten when both halves are done: forgetting one that is still open would be
   * forgetting data the application has not seen (RFC 9000 section 3.3). */
