...
static wt_status_t wt_quic_packet_fail(wt_quic_error_t *out_error, wt_quic_error_t code,
                                       wt_status_t status) {
...
static wt_status_t wt_quic_read_connection_id(wt_cursor_t *c, const uint8_t **out, size_t *out_len,
...
wt_status_t wt_quic_initial_token(const uint8_t *data, size_t length, const uint8_t **out_token,
  ...
  *out_token_length = 0U;
  ...
  if ((wt_quic_packet_type_t)((first & 0x30U) >> 4) != WT_QUIC_PACKET_INITIAL)
    return WT_ERR_PROTOCOL;
   ...
   * not what it is for, and passing NULL here would be a null dereference rather than a skip. */
  {
    ...
    if (wt_quic_read_connection_id(&c, &skipped, &skipped_length, NULL) != WT_OK)
...
wt_status_t wt_quic_long_header_connection_ids(const uint8_t *data, size_t length,
  ...
  if (data == NULL || out_destination == NULL || out_destination_length == NULL ||
      out_source == NULL || out_source_length == NULL) {
...
wt_status_t wt_quic_long_header_decode(wt_cursor_t *c, wt_quic_long_header_t *out,
  ...
  status = wt_quic_read_connection_id(c, &out->destination_connection_id,
                                      &out->destination_connection_id_len, out_error);
  if (status != WT_OK) return status;
  status = wt_quic_read_connection_id(c, &out->source_connection_id, &out->source_connection_id_len,
                                      out_error);
  if (status != WT_OK) return status;
  ...
  if (out->type == WT_QUIC_PACKET_INITIAL) {
    ...
    if (wt_checked_narrow_u64_to_size(token_len, &narrowed) != WT_OK) {
      return wt_quic_packet_fail(out_error, WT_QUIC_FRAME_ENCODING_ERROR, WT_ERR_OVERFLOW);
    }
    if (narrowed > wt_cursor_remaining(c)) return WT_ERR_TRUNCATED;
  ...
  if (length_field < (uint64_t)out->packet_number_len) {
    return wt_quic_packet_fail(out_error, WT_QUIC_FRAME_ENCODING_ERROR, WT_ERR_PROTOCOL);
  }
  {
...
wt_status_t wt_quic_short_header_decode(wt_cursor_t *c, size_t destination_connection_id_len,
  ...
  out->destination_connection_id = wt_cursor_bytes(c, destination_connection_id_len);
  if (out->destination_connection_id == NULL && destination_connection_id_len != 0U) {
    return WT_ERR_TRUNCATED;
  }
