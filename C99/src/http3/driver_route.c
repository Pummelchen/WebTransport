...
static wt_status_t route_quic_frame(wt_http3_driver_t *driver, wt_quic_space_t space,
                                    ...
                                    const wt_http3_driver_sink_t *sink, uint64_t max_frame_bytes,
  ...
  if (frame->kind == WT_QUIC_FRAME_KIND_STREAM) {
    ...
    if (wt_quic_stream_id_is_bidirectional(stream_id)) {
         ...
         * bytes were read as a stream's first and refused. */
      {
        ...
        if (!tracked &&
            (held != NULL || (frame->as.stream.offset == 0U && frame->as.stream.length > 0U))) {
          ...
          classified = wt_http3_driver_classify_bidi_start(assembled, have + copied, &start_kind,
                                                           &prefix_session, &consumed);
          ...
          if (classified == WT_OK && start_kind == WT_HTTP3_BIDI_START_WEBTRANSPORT) {
             ...
             * client never did, so the omission was invisible (WT-156). */
            {
              ...
              wt_status_t remembered =
                  remember_data_stream(driver, stream_id, 0U, prefix_session, 1);
            ...
            if (sink != NULL && sink->on_stream_data != NULL) {
              ...
              if (from_assembled > 0U) {
                wt_status_t delivered = sink->on_stream_data(
                    sink->context, stream_id, assembled + consumed, from_assembled,
                    frame->as.stream.length > copied ? 0 : frame->as.stream.fin);
                if (delivered != WT_OK) return delivered;
              }
              if (frame->as.stream.length > copied) {
                return sink->on_stream_data(sink->context, stream_id,
                                            frame->as.stream.data + copied,
          ...
          if (have > 0U) {
            wt_status_t replayed = wt_http3_driver_on_stream_bytes(
                driver, stream_id, assembled, have, 0, max_frame_bytes, sink, NULL);
       ...
       * two kinds of bytes the type says follow it. */
    {
      ...
      if (finished_prefix) {
        ...
        status = wt_http3_driver_on_uni_stream_data(
            driver, stream_id, frame->as.stream.offset, frame->as.stream.data,
            frame->as.stream.length, &kind, &payload, &payload_length, &consumed, out_error);
        if (status != WT_OK) return status;
      ...
      if (payload_length > 0U || frame->as.stream.fin != 0) {
        status =
            wt_http3_driver_on_stream_bytes(driver, stream_id, payload, payload_length,
                                            frame->as.stream.fin, max_frame_bytes, sink, out_error);
        if (status != WT_OK) return status;
      }
