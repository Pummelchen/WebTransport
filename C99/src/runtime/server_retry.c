...
static wt_status_t read_initial(const uint8_t *datagram, size_t length, retry_initial_t *out,
  ...
  *out_initial = 0;
  ...
  if (wt_quic_long_header_connection_ids(
          datagram, length, &out->destination_connection_id, &out->destination_connection_id_length,
          &out->source_connection_id, &out->source_connection_id_length) != WT_OK) {
    return WT_OK;
  }
...
wt_status_t wt_runtime_server_retry_build(wt_runtime_server_retry_t *retry, const uint8_t *datagram,
  ...
  if (out_length != NULL) *out_length = 0U;
  if (retry == NULL || datagram == NULL || peer == NULL || out == NULL)
    return WT_ERR_INVALID_ARGUMENT;
  ...
  if (header.source_connection_id_length == 0U || header.destination_connection_id_length == 0U)
    return WT_OK;

  address_length = wt_udp_address_encode(peer, address, sizeof(address));
  if (address_length == 0U) return WT_ERR_INVALID_ARGUMENT;
  status = wt_quic_retry_token_build(
      retry->secret, address, address_length, header.destination_connection_id,
      header.destination_connection_id_length, now, token, sizeof(token), &token_length);
  if (status != WT_OK) return status;
  ...
  status = wt_quic_retry_packet_encode(
      &writer, WT_QUIC_VERSION_1, header.source_connection_id, header.source_connection_id_length,
      retry->source_connection_id, retry->source_connection_id_length, token, token_length, tag);
  if (status != WT_OK) return status;
  written = wt_writer_offset(&writer);
  status = wt_quic_retry_integrity_tag(header.destination_connection_id,
                                       header.destination_connection_id_length, out,
...
wt_status_t wt_runtime_server_retry_accept(wt_runtime_server_retry_t *retry,
  ...
  if (retry == NULL || datagram == NULL || peer == NULL || out_original == NULL)
    return WT_ERR_INVALID_ARGUMENT;
  if (retry->armed == 0) return WT_ERR_STATE;
  ...
  address_length = wt_udp_address_encode(peer, address, sizeof(address));
  if (address_length == 0U) return WT_ERR_INVALID_ARGUMENT;
  status = wt_quic_retry_token_validate(retry->secret, address, address_length, now,
                                        retry->token_max_age, header.token, header.token_length,
                                        out_original, capacity, &original_length);
  if (status != WT_OK) {
