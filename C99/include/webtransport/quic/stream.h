...
/* ------------------------------------------------------------------- the table
 *
 * ONE STREAM IS A STATE MACHINE; A CONNECTION HAS MANY, and this is where they live. The table is
 * bounded, which is the resource limit the plan names: a peer chooses how many streams it opens, and a
 * receiver that allocated a structure per stream number would let the peer choose its memory. A table
 * that is full refuses a new stream rather than dropping an old one, because a stream that is silently
 * forgotten is data the application will never see.
 *
 * It owns the stream state and nothing else: no frames, no bytes, no policy about when to send. What it
 * answers is "which stream is this frame about", "may this endpoint open one more", and "which streams
 * are still alive", which is what the receive path needs before it can hand a frame to a state machine.
 */
...
typedef struct wt_quic_stream_table {
  ...
  size_t count;
  /* How many each side has opened, which is what a stream-count limit is checked against. */
...
/* Open a stream. `initiated_by_us` decides which side pays for it and which direction may send.
 * WT_ERR_LIMIT when the table is full or when the opener's limit does not allow one more -- one answer
 * because the caller's reaction is the same (send STREAMS_BLOCKED, or wait), and the two are told apart
 * by reading `wt_quic_stream_table_count` and the counts. WT_ERR_STATE for a stream that is already in
 * the table, which is a caller that has lost track rather than a peer's doing. */
wt_status_t wt_quic_stream_table_open(wt_quic_stream_table_t *table, uint64_t stream_id,
                                      int initiated_by_us, uint64_t limit);

/* The stream with this number, or NULL. A NULL answer is the ordinary case for a frame about a stream
 * this endpoint has not opened and the peer may not have opened either. */
...
/* Forget a stream whose two halves are done, which frees its slot: RFC 9000 section 3.3 lets a peer
 * reuse the number only after both ends have finished with it, so a closed stream is a number that
 * never comes back and a slot that can. WT_ERR_STATE for a stream that is still open. */
wt_status_t wt_quic_stream_table_close(wt_quic_stream_table_t *table, uint64_t stream_id);

/* The live stream at `index`, for a caller that walks them, and the count of live ones. */
...
/* How many streams of one class this endpoint has opened, which is what MAX_STREAMS is raised from. */
 ...
 * the next is the directionality, and the rest is an index within that class. Read in one place so
 * that no rule shifts bits at its call site. */
