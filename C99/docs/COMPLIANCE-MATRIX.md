...
| 5 | A capsule on that stream travels inside an HTTP/3 DATA frame (RFC 9114 section 4.4, RFC 9297 sections 3.1 and 3.2): a DATA frame's payload is what the session's capsule walker reads, a known non-DATA frame is H3_FRAME_UNEXPECTED, and an unknown one is ignored | `wt_http3_frame_data_writer`, `wt_http3_frame_wrap_data_in_place`, `wt_http3_frame_type_is_known`, `wt_http3_driver_is_capsule_stream` | `test_http3_frame`, `test_http3_driver_streams`, `wt_conformance_scenarios` | tested |
...
| 5.4 | A close reason AT the ceiling is accepted: the length bound a receiver enforces is on one capsule rather than on one delivery of them | `wt_webtransport_capsule_decode` | `wt_conformance_scenarios` | tested |
