...
| 4.6 | A stream or datagram that arrives before the session it names is buffered until it can be associated, with a bound on how much | `wt_webtransport_buffered_t`, `wt_http3_driver_data_stream_session_id` | `test_webtransport_buffered`, `test_runtime_session_pair` | tested |
...
| 4.6 | A buffered datagram over the bound is dropped, and a parked item that names another session is dropped rather than refused | `wt_webtransport_buffered_park_datagram`, `wt_webtransport_buffered_drain_datagrams` | `test_webtransport_buffered` | tested |
