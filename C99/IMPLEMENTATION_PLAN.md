...
layers the session runs on -- and prints the nine Definition-of-Done criteria as they stand. Its answer today:
    ...
    26 of 26 draft-16 requirements exercised by a test in this tree
...
| --- | --- | --- |
| Full draft-16 spec matrix implemented | **met** | Every layer is written and tested (Phases 0-9), and `docs/COMPLIANCE-MATRIX.md` maps 26 draft-16 requirements to the implementation and to the test that exercises each one, plus 6 rows for the lower layers. `scripts/check-matrix.sh` verifies that all 46 named symbols still exist and CI runs it, so the document cannot rot away from the code. `WT-132` closed. |
...
| C99 passes the five-implementation VPS interop matrix | **partial** | The C99 tree now has its own interop runner, `scripts/run-container-interop.sh`, and against `pywebtransport`/`aioquic` the client completes the whole session **and the message exchange**: `"connectAccepted":true "responseStatus":200`, the peer logs `stream in: 13 bytes` / `stream echoed`, and the client reports `received 13 byte(s)`. The other two peers this tree can start are measured to a specific point rather than failing silently: `quinn` completes the handshake and never answers the CONNECT (`WT-145`), and `quiche` does not complete the handshake (`WT-146`). The VPS matrix's five implementations still need a host. `WT-135`. |
...
| README status updated from `0%` to the measured final score | **met** | `scripts/score-matrix.sh` prints the score from `docs/COMPLIANCE-MATRIX.md` -- 26 of 26 draft-16 requirements exercised, 7 of the 9 criteria met -- and `C99/README.md` carries that number rather than a guess. `WT-136` closed. |

**What that means for "100%":** seven criteria are met and **two are partially met; none is any longer unmet**. The
