...
| All Swift-equivalent conformance tests pass in C99 | **partial** | The unit suites mirror the Swift suites layer by layer (78 suites, 80,055 checks), the malformed-input corpora exist for QUIC and for HTTP/3/QPACK/session, and the bounded tables are covered. The CONFORMANCE TOOL's scenario count is the remaining gap: **forty-two scenarios** (three codec, eleven refusals, thirteen connection-control, five header and QPACK, two isolation, five interop matrices -- stream, datagram, GOAWAY/close/drain, CONNECT, malformed/flow-control -- the split-prefix edge, and two real sessions). All five of the Swift tools' interop matrices now have a counterpart. Its two remaining scenarios assert PACKAGING FILES rather than library behaviour, so the C99 counterpart belongs in `scripts/check-package.sh` (which installs the tree and builds a consumer against it) rather than in this tool, and that is where they will be mirrored. `WT-133`. |
