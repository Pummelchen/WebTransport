...
| --- | --- | --- |
| Full draft-16 spec matrix implemented | **met** | Every layer is written and tested (Phases 0-9), and `docs/COMPLIANCE-MATRIX.md` maps 24 draft-16 requirements to the implementation and to the test that exercises each one, plus 5 rows for the lower layers. `scripts/check-matrix.sh` verifies that all 40 named symbols still exist and CI runs it, so the document cannot rot away from the code. `WT-132` closed. |
| All Swift-equivalent conformance tests pass in C99 | **partial** | The unit suites mirror the Swift suites layer by layer (79 suites, 80,156 checks), the malformed-input corpora exist for QUIC and for HTTP/3/QPACK/session, and the bounded tables are covered. The audit found ONE real feature gap rather than a documentation one: draft-16 section 3.2's sub-protocol negotiation (`wt-protocol`, Structured Fields) had no code here at all, and `src/webtransport/protocol.c` now carries the codec AND the session decision -- the token rules, the first-offered-that-is-supported selection, the required-selection refusal (400) and the client's check that a response names a token it offered -- with `test_webtransport_protocol` (101 checks). What is missing is the APPEND path: `wt_http3_message_encode` writes pseudo-headers only, so nothing puts the field into a request or response section yet, and the matrix gets a row only when a session negotiates one end to end. The CONFORMANCE TOOL's scenario count is the other gap: **forty-three scenarios** (three codec, eleven refusals, thirteen connection-control, five header and QPACK, two isolation, five interop matrices -- stream, datagram, GOAWAY/close/drain, CONNECT, malformed/flow-control -- a seven-case flow-control matrix, the split-prefix edge, and two real sessions). All five of the Swift tools' interop matrices now have a counterpart. Its two remaining scenarios assert PACKAGING rather than library behaviour, and they are now mirrored where that belongs: `scripts/check-package.sh` installs the tree with the tools on and asserts that the product list is the three tools -- and that no test, spike or sample binary came with them. Writing that check found the gap it exists for: the install rules carried the libraries and headers but not the tools, so the CLIs were products of the build and not of the package, and the rule now names them. `WT-133`. |
...
| README status updated from `0%` to the measured final score | **met** | `scripts/score-matrix.sh` prints the score from `docs/COMPLIANCE-MATRIX.md` -- 24 of 24 draft-16 requirements exercised, 6 of the 9 criteria met -- and `C99/README.md` carries that number rather than a guess. `WT-136` closed. |
