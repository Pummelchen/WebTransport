...
layers the session runs on -- and prints the nine Definition-of-Done criteria as they stand. Its answer today:
    ...
    34 of 34 draft-16 requirements exercised by a test in this tree
...
| --- | --- | --- |
| Full draft-16 spec matrix implemented | **met** | Every layer is written and tested (Phases 0-9), and `docs/COMPLIANCE-MATRIX.md` maps 31 draft-16 requirements to the implementation and to the test that exercises each one, plus 6 rows for the lower layers. `scripts/check-matrix.sh` verifies that all 58 named symbols still exist and CI runs it, so the document cannot rot away from the code. `WT-132` closed. |
