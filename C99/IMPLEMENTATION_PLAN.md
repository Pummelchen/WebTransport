...
layers the session runs on -- and prints the nine Definition-of-Done criteria as they stand. Its answer today:
    ...
    37 of 38 draft-16 requirements exercised by a test in this tree
    1 of 38 partial, and 6 row(s) describing lower layers
    7 of 9 criteria met, 2 partial, 0 not met

The README carries exactly that, with the distinction stated where a reader cannot miss it: **100% of the matrix
is not 100% done.** The two partial criteria are outside the matrix: the five-implementation interop matrix
reproduces 5 of its 7 proofs on a host, and the FreeBSD and Windows CI legs still lack a FreeBSD job (their
portability work is done). Writing one number and letting it
...
| --- | --- | --- |
| Full draft-16 spec matrix implemented | **met** | Every layer is written and tested (Phases 0-9), and `docs/COMPLIANCE-MATRIX.md` maps 38 draft-16 requirements to the implementation and to the test that exercises each one, plus 6 rows for the lower layers -- 44 data rows in all: 40 `tested`, 2 `partial` and 2 `--`. `scripts/check-matrix.sh` verifies that all 95 named symbols/tests still exist and CI runs it, so the document cannot rot away from the code. `WT-132` closed. |
...
| C99 passes the five-implementation VPS interop matrix | **partial** | Five of the seven Phase 11 proofs reproduce against five independent implementations on a routable host, with `--trust system` so the chain is validated against the platform trust store and the certificate's name is checked: pywebtransport/aioquic (stream), web-transport-quinn (stream and datagram), web-transport-quiche (stream) and hyperium/h3-webtransport (datagram). The two `erlang-webtransport` proofs (stream and datagram) fail `status=trust` while the other four peers accept the same certificate files, and an RSA certificate tried on the theory that the peer needed one made the other four fail, which excludes key type and leaves that peer's own TLS handling (`WT-196`). `scripts/run-vps-third-party-interop.sh` counts the aggregate from the proof files rather than asserting it. Two of the five peers needed a patch to interoperate, both kept under `tests/interop/peer/`. `WT-135` built the matrix and is closed; the 7 of 7 an earlier run reported is not reproducible until `WT-196` is fixed. |
| CI is green on macOS 26, Debian, FreeBSD and Windows 11 | **partial** | `c99-ci.yml` builds and tests on macOS and Ubuntu 24.04 (the matrix's two `ubuntu-24.04` legs; the last completed run was green), with sanitizers, the package consumer and the CLI smoke. A `linux-debian13` leg runs the suite in a `debian:trixie` container, so the platform this criterion names is now run by a job of its own (`WT-225`). The Windows legs are now JOBS and the native one is enforced (`WT-224`), while the FreeBSD leg is still absent; and the portability work they needed is done rather than planned: the `_WIN32` branch compiles and links the whole tree (91 PE32+ executables and one shared library) and RUNS under Wine with its suite green (**85 of 85 test executables, 64,900 checks**), including a Windows-only test that asserts the datagram layer's contract on both of its receive paths; FreeBSD 15.1 builds the tree with its base clang and passes the suite once `python3` is installed, which the first pass found by failing 8 of 96 for that reason alone (`WT-201`, a harness dependency rather than a protocol defect). What remains is a runner GitHub does not provide natively — a FreeBSD job (`WT-223`) — and the plan's MSVC and Clang-CL Windows variants, which no job covers; those are rows on the [C99 tracker](https://github.com/Pummelchen/WebTransport/wiki/Project-Tracker-C99), which is the single list of open work for this tree. |
...
| README status updated from `0%` to the measured final score | **met** | `scripts/score-matrix.sh` prints the score from `docs/COMPLIANCE-MATRIX.md` -- 37 of 38 draft-16 requirements exercised, 7 of the 9 criteria met -- and `C99/README.md` carries that number rather than a guess. That claim was false when it was written: the README's headline still read `Draft-16 score: 0%` and `Phases 6 to 14 are not started` while this row said otherwise, and a public reference library whose first paragraph is wrong is worse than one that says nothing. The headline and the closing "what is not here yet" paragraph now state the measured position. `WT-136` closed. |

**What that means for "100%":** seven criteria are met and **two are partially met; none is unmet**. The
remaining work is named with numbers, and none of it is blocked by a defect here: the CI leg that has to run on
FreeBSD and Windows (`WT-134`, whose portability work is now DONE -- the branch compiles, links and RUNS under
Wine with its whole suite green, and what is missing is the JOB), and the interop matrix, which is **partial**:
5 of its 7 Phase 11 proofs reproduce against five independent implementations on a routable host, and the
`erlang-webtransport` stream and datagram proofs fail `status=trust` (`WT-196`, that peer's own TLS handling).
The matrix (`WT-132`), the score that follows it (`WT-136`) and the conformance-coverage audit (`WT-133`) have
all landed, which is why the count moved from four to seven.
