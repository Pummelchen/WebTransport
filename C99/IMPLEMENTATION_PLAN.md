...
| All Swift-equivalent conformance tests pass in C99 | **partial** | The unit suites mirror the Swift suites layer by layer (78 suites, 80,055 checks), the malformed-input corpora exist for QUIC and for HTTP/3/QPACK/session, and the bounded tables are covered. The CONFORMANCE TOOL's scenario count is the remaining gap: **forty-one scenarios** (three codec, eleven refusals, thirteen connection-control, five header and QPACK, two isolation, four interop matrices -- stream, datagram, GOAWAY/close/drain, CONNECT -- the split-prefix edge, and two real sessions). Four of the Swift tools' five interop matrices now have a counterpart; its malformed-flow matrix and its two release checks do not, while this report carries refusal and control cases the Swift suite folds into other groups. `WT-133`. |
