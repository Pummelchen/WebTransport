...
| All Swift-equivalent conformance tests pass in C99 | **partial** | The unit suites mirror the Swift suites layer by layer (78 suites, 80,055 checks), the malformed-input corpora exist for QUIC and for HTTP/3/QPACK/session, and the bounded tables are covered. The CONFORMANCE TOOL's scenario count is the remaining gap: **thirty-nine scenarios** (three codec, eleven refusals, thirteen connection-control, five header and QPACK, two isolation, two interop matrices (eleven stream cases, eight datagram cases), the split-prefix edge, and two real sessions) against the Swift tools' 40-scenario suite, whose interop-matrix, release and multi-session groups still have no counterpart here. `WT-133`. |
