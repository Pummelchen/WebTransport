...
| All Swift-equivalent conformance tests pass in C99 | **partial** | The unit suites mirror the Swift suites layer by layer (78 suites, 80,055 checks), the malformed-input corpora exist for QUIC and for HTTP/3/QPACK/session, and the bounded tables are covered. The CONFORMANCE TOOL's scenario count is the remaining gap: **forty scenarios** (three codec, eleven refusals, thirteen connection-control, five header and QPACK, two isolation, three interop matrices -- stream, datagram, GOAWAY/close/drain -- the split-prefix edge, and two real sessions). The COUNT now matches the Swift tools' 40-scenario suite and the SETS do not: its connect and malformed-flow interop matrices and its two release checks still have no counterpart here, while this report carries refusal and control cases the Swift suite folds into other groups. `WT-133`. |
