...
layers the session runs on -- and prints the nine Definition-of-Done criteria as they stand. Its answer today:
    ...
    8 of 9 criteria met, 1 partial, 0 not met

The README carries exactly that, with the distinction stated where a reader cannot miss it: **100% of the matrix
is not 100% done.** The one partial criterion is outside the matrix: the FreeBSD and Windows CI legs still lack a
FreeBSD job (their portability work is done). Writing one number and letting it
...
Wine with its whole suite green, and what is missing is the JOB). The interop matrix is **met** again: all seven
...
all landed, which is why the count moved from four to seven, and the `WT-196` fix takes it to eight.
