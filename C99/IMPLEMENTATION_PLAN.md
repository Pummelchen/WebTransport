...
**The adaptation is DONE and the jobs are IN.** The order this paragraph used to state as a plan -- the private
platform header, `WSAStartup` ownership, the peek difference, then the job -- is complete: the header is in and
RUN, `WSAStartup` is owned by the socket lifetime, the peek difference is measured and documented rather than
assumed, and the jobs run what the measurements said they would -- `check-windows-build.sh` (91 PE32+
executables) on the Ubuntu leg and natively on `windows-latest`, and `check-windows-wine.sh` (85 of 85 green)
under Wine (`WT-224`). A CI job that cannot pass teaches everyone to ignore CI, which is why none was added
while the branch was unverified; a job that runs two scripts known to pass is a different thing. The FreeBSD
leg went the same way: it was measured by hand on a real kernel first, and `WT-223` then booted that kernel in a
VM on an Ubuntu runner so a FreeBSD-only regression can fail a pull request.
...
layers the session runs on -- and prints the nine Definition-of-Done criteria as they stand. Its answer today:
    ...
    39 of 40 draft-16 requirements exercised by a test in this tree
    1 of 40 partial, and 6 row(s) describing lower layers
    9 of 9 criteria met, 0 partial, 0 not met

The README carries exactly that, with the distinction stated where a reader cannot miss it: **100% of the matrix
is not 100% done** -- one draft-16 requirement is still `partial`, and the plan's Phase 12 matrix still lacks its
two Windows compiler variants (MSVC and Clang-CL; the legs use mingw GCC), which is a tracker row rather than a
criterion. Writing one number and letting it
...
| --- | --- | --- |
| Full draft-16 spec matrix implemented | **met** | Every layer is written and tested (Phases 0-9), and `docs/COMPLIANCE-MATRIX.md` maps 40 draft-16 requirements to the implementation and to the test that exercises each one, plus 6 rows for the lower layers -- 46 data rows in all: 42 `tested`, 2 `partial` and 2 `--`. `scripts/check-matrix.sh` verifies that all 95 named symbols/tests still exist and CI runs it, so the document cannot rot away from the code. `WT-132` closed. |
