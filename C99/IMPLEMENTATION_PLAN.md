...
layers the session runs on -- and prints the nine Definition-of-Done criteria as they stand. Its answer today:
    ...
    30 of 30 draft-16 requirements exercised by a test in this tree
...
| --- | --- | --- |
| Full draft-16 spec matrix implemented | **met** | Every layer is written and tested (Phases 0-9), and `docs/COMPLIANCE-MATRIX.md` maps 30 draft-16 requirements to the implementation and to the test that exercises each one, plus 6 rows for the lower layers. `scripts/check-matrix.sh` verifies that all 56 named symbols still exist and CI runs it, so the document cannot rot away from the code. `WT-132` closed. |
...
| README status updated from `0%` to the measured final score | **met** | `scripts/score-matrix.sh` prints the score from `docs/COMPLIANCE-MATRIX.md` -- 30 of 30 draft-16 requirements exercised, 7 of the 9 criteria met -- and `C99/README.md` carries that number rather than a guess. `WT-136` closed. |
