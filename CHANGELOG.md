...
- `swift-tools-version` stays at `6.3` deliberately: it declares the oldest SwiftPM that may read the manifest, and the project's floor is Swift 6.3.3 / Xcode 26.6, while the mandated toolchain is asserted by CI. The reason is recorded in the manifest rather than left to look stale.
