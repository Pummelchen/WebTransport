...
- **The CONNECT-stream capsule reader could be driven into remote memory exhaustion.** A peer's declared capsule payload was buffered whole before anything checked it, so one `WT_CLOSE_SESSION` header could ask for an allocation bounded only by the peer's imagination. `maximumConnectStreamCapsulePayloadBytes` (1028 — `WT_CLOSE_SESSION`'s 4-byte error code plus draft-16's 1024-byte message cap) is now enforced *before* the payload is buffered, and an oversized capsule becomes an `H3_MESSAGE_ERROR` on that stream rather than an allocation.
- **A peer's HTTP/3 field values are validated.** CR, LF, NUL, the other C0 controls and DEL are refused in a field value, covering decoded, dynamic-table and caller-supplied lines, and `:status` must be exactly three digits. Before, a value this layer accepted could become a second field in a later writer — the classic intermediary-encapsulation shape.
- **A stream half this endpoint does not own is no longer signalled or reset.** `resetStream` refuses a stream whose send half the endpoint does not own and `stopSendingStream` one whose receive half it does not own, both with `QUICStateError.streamStateViolation`, and session teardown resets and stops only the halves it holds (RFC 9000 section 2.1).
- **`FINAL_SIZE_ERROR` is reachable.** The final-size checks in the stream state ran after the receive-closed gate that already returned, so the two RFC 9000 section 4.5 branches could not fire; an inbound `RESET_STREAM` is now accepted and its Final Size recorded.
- **The QUIC short-header decoder validates its reserved bits**, as the long-header and Retry decoders always did.
- **CRYPTO-stream reassembly is linear and bounded.** It recomputed a consumed-byte count by re-summing the buffered rows and retained every consumed byte forever — quadratic in the buffer and unbounded in time. The pending count is stored and the conflict-detection history is a 4 KiB window, with the TLS transcript capped at 1 MiB.
- **The HKDF-Expand-Label block counter no longer wraps.** The CryptoKit copy allowed an output length past RFC 5869's 255-block cap that the TLS-core copy already guarded, so a requested length could silently produce a short or wrong derivation instead of an error (8160 bytes succeed, 8161 is refused).
- **A peer's echo text is escaped in operator output.** The CLI print sites interpolated the peer's `result.message` raw, so a peer could forge log lines; they now use the new `WebTransportLogText.escaped(_:)`.
- **An Ed25519 key name no longer maps to an EC key type.** The test-support parser answered `kSecAttrKeyTypeECSECPrimeRandom` for `"ed25519"`, so an Ed25519 key was loaded as P-256; it now takes the same unsupported-key-type refusal as any other unknown name.
- **The C99 tree's S0 set.** Ten QPACK static-table entries were truncated at RFC 9204's line wrap, and the `--check` written to catch that re-derived the same error and so could not fail; the draft-16 `:protocol` token was defined as the HTTP/2 capsule token, so `webtransport-h3` was neither sent nor accepted; and the stream table's reclaim decremented the ID-issuing counters, so a session reused stream IDs and then failed.
- **C99 protocol and bounds defects.** The `reset_stream_at` transport-parameter codepoint never negotiated the extension draft-16 requires; two RFC 9000 section 18.2 MUSTs on transport-parameter values were unchecked; a peer's `initial_source_connection_id` was adopted as the destination connection ID without being compared to the TLS handshake; a WebTransport unidirectional prefix split between the type and the session ID aborted the connection; pseudo-header values were never grammar-checked; capsules arriving after `WT_CLOSE_SESSION` were still applied; a released frame-state slot could be written through a stale pointer, desynchronising another stream; the HTTP/3 field-name and field-value grammar was unenforced; the Windows monotonic clock wrapped after about 21 days of uptime because ticks were multiplied before dividing; `wt_alloc_array` reported success while returning `NULL`; and `wt_http3_endpoint_write_prefix` reported success without checking that the prefix was written.
...
- `WebTransportSession.acceptUnidirectionalStream(maximumInitialBytes:)` and `WebTransportUnidirectionalStream`, with the runtime equivalents `WebTransportNetworkSession.acceptUnidirectionalStream(...)` and `WebTransportNetworkUnidirectionalStream`. A peer-initiated unidirectional stream at last has a consumer: it was previously queued and never delivered, and the queue grew until it refused. The new type is **receive only** — it exposes the stream identifier and `receive(maximumBytes:)` and has no send path — and a prefix naming another session is refused with `sessionGone` before the stream is registered or buffered, as the bidirectional path already did.
- `WebTransportLogText.escaped(_:)`, which escapes backslash, quote, CR, LF, HTAB and every other C0 control and DEL, for embedding peer-controlled text in operator output.
- `Swift/check-target-imports.sh`, a blocking CI check that every module a target imports is covered by its declared dependencies — the defect class that left the Swift 6.4 test bundle unlinkable.
- On the C99 side, `--upgrade-token draft16|legacy` on `wt-client-c99` (draft-16 by default) selects the CONNECT's `:protocol` token, so the pre-draft `webtransport` token is available for the peers that still require it. `--help` and the JSON report name the selection, and an unknown value is refused.
- `THIRD_PARTY_NOTICES.md`, carrying the Apache-2.0 notice for OpenSSL, the C99 build's required dependency.
- A blocking `security-scan` workflow: gitleaks over the full commit history and trivy over the tree, both pinned and checksum-verified, where an absent scanner fails the job instead of passing it.
...
- `datagramsAvailable` — and therefore the capability `echo()` reports — now follows `SETTINGS_H3_DATAGRAM` as advertised by both endpoints instead of being a constant `true`, so a peer that did not negotiate datagrams reads as false. A caller that ignored the flag and used datagrams anyway keeps working exactly as far as the peer allows.
- `receive(maximumBytes:)` honours its bound when the stream already holds a buffered initial payload, returning at most that many bytes and keeping the remainder for the next read, rather than handing back the whole buffer.
- `WebTransportQUICServer` and `WebTransportServer.listen(on:)` take `maxConcurrentConnections: Int?`, with `nil` as the only "not supplied": an explicit `16` is no longer read as the old `16`-means-default sentinel.
- The per-connection inbound-stream queue is bounded by the advertised WebTransport stream limits (at least one slot per direction) and a full queue is refused explicitly with `refusedQueueFull(limit:)` instead of growing without bound; session teardown resets the streams and halves this endpoint owns and no others.
- Both manifests build with `.treatAllWarnings(as: .error)` and `.strictMemorySafety()`, the manifests themselves are under the `swift format lint --strict` gate, and CI runs on Xcode 27 / Swift 6.4: it additionally builds and runs the nested manifest's own test target, its library smoke pair and the CLI conformance tool.
- `WebTransportServer --timeout-ms` refuses a non-positive value with a message naming the requirement, as the C99 CLI already did, instead of accepting a value it then ignores.
- The C99 CONNECT's `:protocol` is `webtransport-h3` for draft-16, with the pre-draft token only in the explicit legacy mode described above.
- Repository hygiene: 185 files of committed CMake/Ninja build output left the tree and the build directories are ignored; every workflow action is pinned to a commit SHA and declares least-privilege `permissions:`; the supported-versions table names the current 1.3.x line; the README's Windows CI statements match the two legs that exist; and the hand-maintained "Views (14d)" badge and its committed `traffic.json` snapshot are gone — the file was refreshed by hand and no workflow regenerated it, so it advertised a frozen number as a live one.
- The Swift toolchain baseline is Swift 6.4 / Xcode 27, which supersedes the audit's `A-0005` decision to leave `swift-tools-version` at `6.3`: the floor and the mandate are now the same toolchain, so both manifests declare `swift-tools-version: 6.4`, `Swift/check-toolchain.sh` defaults to Swift 6.4 / Xcode 27 (CI passes the same floors explicitly), the macOS jobs run on the `xcode-27` image, and the README states the floor. Two things the 6.4 compiler made explicit were fixed rather than suppressed: 12 redundant `unsafe` effect markers are gone — `[#UnnecessaryEffectMarker::UnnecessaryUnsafe]` reports an `unsafe` expression with no unsafe operation inside, so removing it is what the diagnostic asks for — and `WebTransportHTTP3CoreTests` depends on `WebTransportTLSCore` explicitly, because the 6.4 build system no longer resolves that transitive symbol and the bundle failed to link with "Undefined symbols for architecture arm64".
- Swift is no longer part of this repository's CodeQL default setup. That setup autobuilds with the runner image's Swift 6.3.3, which cannot parse a 6.4 manifest; advanced setup on `xcode-27` with `build-mode: manual` (the pattern MCPSearch uses) is the way to bring Swift CodeQL back.
